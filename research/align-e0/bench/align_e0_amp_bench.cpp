// ALIGN-E0 application amplifier (#265 Phase 3, prereg §9): the realistic
// copy pipeline end-to-end with the exposed buffer alignment as the only
// variable, vs the REAL production engine as external fidelity reference.
//
//   --arm engine    the REAL production engine
//                   (run_pipelined_copy_with_backend — the same
//                   copy_task.cpp the CLI uses). Its slot storage is the
//                   fixed production representation (natural pointer);
//                   external consistency reference only.
//   --arm natural   research replica of the production copy task (verbatim
//                   algorithm) with exposed buffer = base + 16 (16-aligned,
//                   page offset 16; glibc-chunk-user-pointer shape).
//   --arm aligned   the same replica with exposed buffer rounded up to
//                   --align bytes (page offset 0).
//
// The replica's backing is ONE over-allocated owned block per slot
// (posix_memalign(&base, 4096, cap + 4096)), identical mechanism across
// arms; only the exposed pointer changes (the ALIGN-E0 §1 core principle).
// Production code is NOT modified (research-only replica).
//
// Per rep: slots are constructed (replica arms; timed separately), then the
// full engine call is timed (Runtime build/start/submit/wait/drain/join +
// the copy). Same-work gates (fail-closed): bytes_copied == file size,
...
[[noreturn]] void amp_fatal(const char* what, int err) {
    std::fprintf(stderr, "align_e0_amp_bench: fatal: %s (errno=%d: %s)\n",
                 what, err, std::strerror(err));
    std::exit(2);
}

[[noreturn]] void amp_semantic(const char* what) {
    std::fprintf(stderr, "align_e0_amp_bench: semantic failure: %s\n",
                 what);
    std::exit(3);
}
...
enum class Arm { engine, natural, aligned };

const char* arm_name(Arm a) {
    switch (a) {
    case Arm::engine: return "engine";
    case Arm::natural: return "natural";
    case Arm::aligned: return "aligned";
    }
    return "?";
}
...
// the selectable exposed buffer alignment (ReplicaSlot). Keep this in
// lockstep with the production file when comparing results; the engine vs
// replica pair exists precisely to detect any drift (production natural
// pointer ~ replica natural arm).
// ===========================================================================
...
struct ReplicaSlot {
    // Backing: ONE over-allocated owned block per slot, page-aligned base,
    // identical mechanism for every arm. Exposed pointer = base + 16
    // (natural) or round_up(base, align) (aligned). Only the exposed
    // pointer differs between arms.
    ...
    explicit ReplicaSlot(std::size_t cap, std::size_t align) {
        alloc_cap = cap + 4096;
        ...
        if (align == 16) {
            storage = reinterpret_cast<std::byte*>(raw + 16);
        } else {
            storage = reinterpret_cast<std::byte*>(round_up(raw, align));
        }
        if (reinterpret_cast<std::uintptr_t>(storage) + cap >
            raw + alloc_cap)
            amp_semantic("exposed buffer exceeds owned block");
    }
...
struct ReplicaCopyTask {
    ...
    std::size_t alignment;  // the ONLY delta vs the production task
...
struct Config {
    Arm arm = Arm::natural;
    std::size_t alignment = 4096;  // for --arm aligned
...
Config parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--arm") {
            ...
            if (p == "engine") c.arm = Arm::engine;
            else if (p == "natural") c.arm = Arm::natural;
            else if (p == "aligned") c.arm = Arm::aligned;
            else amp_semantic("bad --arm");
        } else if (a == "--align") {
            c.alignment = std::strtoull(next("--align").c_str(), nullptr, 10);
        } else if (a == "--buffer-size") {
            c.buffer_size =
                std::strtoull(next("--buffer-size").c_str(), nullptr, 10);
        } else if (a == "--depth") {
...
int run_amp(const Config& cfg) {
    ...
    for (std::size_t rep = 0; rep < cfg.reps; ++rep) {
        ...
        if (cfg.arm == Arm::engine) {
            ...
            const std::size_t align =
                cfg.arm == Arm::natural ? 16 : cfg.alignment;
    ...
    std::string out = "{\n";
    out += "  \"bench\": \"align_e0_amp_bench\",\n";
    out += std::string("  \"arm\": \"") + arm_name(cfg.arm) + "\",\n";
    out += "  \"alignment\": " + std::to_string(cfg.alignment) + ",\n";
...
int main(int argc, char** argv) {
    ...
    if (cfg.arm == Arm::aligned &&
        (cfg.alignment == 0 || (cfg.alignment & (cfg.alignment - 1)) != 0))
        amp_semantic("--align must be a nonzero power of two");
    return run_amp(cfg);
}
