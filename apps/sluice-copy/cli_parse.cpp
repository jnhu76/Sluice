...
int usage(const char* prog) {
    std::fprintf(stderr,
                 ...
                 static_cast<std::size_t>(kMaxPipelineBytes), static_cast<unsigned>(kMaxWorkers));
    return 1;
}
...
int parse_args(int argc, char** argv, CliArgs& args) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--help") {
            ...
            if (!v || !parse_size(v, args.pipeline_depth))
                return usage(argv[0]);
        } else if (a == "--workers") {
...
const char* code_name(sluice::IoError::Code c) {
    switch (c) {
    case sluice::IoError::Code::eof:
        return "eof";
    case sluice::IoError::Code::canceled:
        return "canceled";
    case sluice::IoError::Code::no_space:
        return "no_space";
    case sluice::IoError::Code::permission_denied:
        return "permission_denied";
    case sluice::IoError::Code::invalid_state:
        return "invalid_state";
    case sluice::IoError::Code::backend_error:
