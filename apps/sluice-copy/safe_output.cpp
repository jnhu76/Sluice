...
SafeOpenOutcome open_atomic_copy(const std::string& src_path, const std::string& dst_path) {
    ...
    if (!S_ISREG(src_stat.st_mode)) {
        return fail(SafeOpenFailure::src_not_regular, IoError{IoError::Code::invalid_state});
    }

    struct stat dst_stat{};
    if (::stat(dst_path.c_str(), &dst_stat) == 0) {
        if (!S_ISREG(dst_stat.st_mode)) {
            return fail(SafeOpenFailure::dst_not_regular, IoError{IoError::Code::invalid_state});
        }
        if (src_stat.st_dev == dst_stat.st_dev && src_stat.st_ino == dst_stat.st_ino) {
            return fail(SafeOpenFailure::same_file, IoError{IoError::Code::invalid_state});
        }
    } else if (errno != ENOENT && errno != ENOTDIR) {
        return fail(SafeOpenFailure::dst_stat, sluice::from_errno_value(errno));
    }
    ...
    if (temp_fd < 0) {
        return fail(SafeOpenFailure::temp_create, sluice::from_errno_value(errno));
    }
    ...
    if (::fchmod(temp_fd, mode) != 0) {
        ...
        return fail(SafeOpenFailure::temp_chmod, sluice::from_errno_value(errno));
    }
...
const char* safe_open_failure_message(SafeOpenFailure f) {
    switch (f) {
    case SafeOpenFailure::none:
        return "ok";
    case SafeOpenFailure::src_open:
        return "cannot open source";
    case SafeOpenFailure::src_stat:
...
sluice::Result<void> commit_atomic_copy(SafeOpenOutcome& o, const std::string& dst_path,
                                        SyncPolicy sync, SafeCommitStage* stage) {
    if (stage)
        *stage = SafeCommitStage::none;
    ...
    if (sync != SyncPolicy::none) {
        ...
        if (dir_fd < 0) {
            if (stage)
                *stage = SafeCommitStage::dir_sync;
            return sluice::make_unexpected<void>(sluice::from_errno_value(errno));
        }
        ScopedFd dir_guard(dir_fd);

        int rc = sluice::detail::retry_on_eintr([&] { return directory_fsync(dir_fd); });
        if (rc != 0) {
            if (stage)
                *stage = SafeCommitStage::dir_sync;
            return sluice::make_unexpected<void>(sluice::from_errno_value(errno));
