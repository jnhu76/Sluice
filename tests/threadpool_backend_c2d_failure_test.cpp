...
//      exactly one winner, no overwrite, no double publication, exactly one
//      tally, no worker execution in any outcome.
//
...
// INVARIANT asserted is: exactly one terminal, exactly one publication, no
// worker execution, and exactly one tally in either outcome — a request can
// never have two terminals or two published results.
SLUICE_TEST_CASE(tp_c2d_dispatch_failure_races_cancel_exactly_one) {
    ...
    // Exactly one tally: canceled XOR dispatch-failure (completion_errors is
    // not wired for ThreadPool — the tally authority here is canceled_ops).
