...
#if !defined(SLUICE_HAS_LIBURING)
SLUICE_TEST_CASE(uring_io_context_stub_returns_backend_error_without_liburing) {
    sluice::experimental::UringIoContext ctx(8);
    std::vector<std::byte> buf(4, std::byte{0xAB});
    auto r = ctx.write_file_all("/tmp/sluice_uring_stub.tmp", std::span<const std::byte>(buf));
...
SLUICE_TEST_CASE(uring_io_context_invalid_path_errors) {
    sluice::experimental::UringIoContext ctx(8);
