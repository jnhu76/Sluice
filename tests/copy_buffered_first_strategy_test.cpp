...
sluice::CopyOptions opts_with(sluice::CopyStrategy s,
                              sluice::CopyLimit lim = sluice::CopyLimit::unlimited()) {
...
SLUICE_TEST_CASE(buffered_first_respects_limit_smaller_than_buffered) {
    ...
    auto res = sluice::copy_all(
        br, writer, std::span<std::byte>(scratch),
        opts_with(sluice::CopyStrategy::BufferedFirst, sluice::CopyLimit::bytes(5)), nullptr, &dec);
    SLUICE_CHECK(res.has_value());
...
SLUICE_TEST_CASE(buffered_first_nothing_limit_touches_nothing) {
    ...
    auto res = sluice::copy_all(
        br, writer, std::span<std::byte>(scratch),
        opts_with(sluice::CopyStrategy::BufferedFirst, sluice::CopyLimit::nothing()), nullptr,
        &dec);
    SLUICE_CHECK(res.has_value());
