...
// unavailable (CPPIO_HAS_LIBURING not defined), the test asserts the clean
...
#if !defined(CPPIO_HAS_LIBURING)
CPPIO_TEST_CASE(uring_write_batch_stub_returns_backend_error_without_liburing) {
...
CPPIO_TEST_CASE(uring_write_batch_writes_small_file) {
    TempPath tp;
...
CPPIO_TEST_CASE(uring_write_batch_invalid_fd_errors) {
    cppio::experimental::UringWriteBatch batch(8);
