...
sluice::CopyOptions opts_deferred(
    sluice::CopyStrategy s,
...
SLUICE_TEST_CASE(deferred_strategy_fills_decision) {
    ...
    auto res =
        sluice::copy_all(reader, writer, std::span<std::byte>(scratch),
                         opts_deferred(sluice::CopyStrategy::SendfileDeferred), nullptr, &dec);
    SLUICE_CHECK(!res.has_value());
