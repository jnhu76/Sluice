...
-- Deterministic injection seams prove: (1) pre-commit rejection on the REAL
-- backend is transactional (binding-CAS loss -> invalid_state, zero residue,
-- capacity immediately recyclable); (2) partial worker-startup failure stops
-- and joins the already-started workers and rethrows synchronously (finding
-- P1-04); (3) a post-commit permanent dispatch failure (injected between
-- enqueue and dispatch push, inside work_mtx_, with no worker ever able to
-- see the handle) leaves submit successful, drives the request to exactly ONE
-- defined backend_error terminal, publishes once via reap, keeps the borrow
-- active until reap, and never executes a worker or syscall — for BOTH the
-- size and void operation paths; (4) the accepted submit -> enqueue/terminal
-- -> reap -> reset path performs ZERO heap allocations under an always-throw
-- operator new (ADR Decision 14 / I9) on the real worker path and on the
-- injected failure path; (5) the dispatch-failure terminal vs cancel has
-- exactly one winner, no overwrite, no double publication, and exactly one
-- tally in every interleaving. Gated to linux/macosx (POSIX syscalls).
