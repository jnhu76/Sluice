...
-- e15_public_api_acceptance — public-only acceptance consumer (E15 §17). Uses
-- INSTALLED/PUBLIC headers only (no tests/ include path, no
-- SLUICE_ASYNC_INTERNAL_TESTING, no private source). Exercises Result +
-- AsyncIoContext + Completion + Batch end-to-end against the production
-- sluice_async. Build + run proves the public async-foundation surface is
-- usable from a consumer that sees only the installed headers.
do
    local R = SLUICE_ROOT
    local p = R .. "examples/e15_public_api_acceptance.cpp"
    if os.isfile(p) then
        target("e15_public_api_acceptance")
            set_kind("binary")
            set_default(false)
            set_group("examples")
