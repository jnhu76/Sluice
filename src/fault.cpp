...
Result<std::size_t> FaultReader::read_some(std::span<std::byte> dst) {
    ...
    if (plan_.fail_after_bytes) {
        std::uint64_t budget =
            *plan_.fail_after_bytes > bytes_seen_ ? *plan_.fail_after_bytes - bytes_seen_ : 0;
        window =
            dst.first(static_cast<std::size_t>(std::min<std::uint64_t>(window.size(), budget)));
    }
...
Result<std::size_t> FaultWriter::write_some(std::span<const std::byte> src) {
    ...
    if (plan_.fail_after_bytes) {
        std::uint64_t budget =
            *plan_.fail_after_bytes > bytes_seen_ ? *plan_.fail_after_bytes - bytes_seen_ : 0;
        window =
            src.first(static_cast<std::size_t>(std::min<std::uint64_t>(window.size(), budget)));
    }
...
Result<void> FaultWriter::flush() {
    if (plan_.fail_flush) {
        return make_unexpected<void>(plan_.error);
    }
    return inner_.flush();
}
