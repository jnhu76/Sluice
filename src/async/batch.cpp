...
std::size_t Batch::add(BatchOp op) {
    const bool is_void =
        (op.kind == BatchOp::Kind::sync_data || op.kind == BatchOp::Kind::sync_all);
...
std::optional<BatchResult> Batch::next() noexcept {
    ...
    r.origin =
        s.submit_rejected ? BatchResultOrigin::rejected : BatchResultOrigin::accepted_and_completed;
    r.is_void = s.is_void;
    if (s.is_void) {
