...
// Stage-boundary fail-fast: a due ACTIVE SelectTimerRegistration is
// unreachable in valid production state. Terminates the process; never
// returns. See include/sluice/async/detail/fail_fast.hpp.
[[noreturn]] void select_timer_pump_active_fail_fast() noexcept {
    std::terminate();
}

// Stage-boundary fail-fast: a single Event::set() broadcast observed arms
// belonging to more than one distinct eligible SelectGroup. Multi-group
// shared Events are DENIED at this boundary; fail fast before any group
...
// Group lifetime fail-fast. ~Group with pending Evented task.
...
// AsyncIoContext destroyed or move-assigned over while
...
// Invalid runnable-ticket consumption. run_next_on consumed a ticket
...
// Invalid suspend transition. commit_suspend_locked attempted
...
// Wait-registry invariant violation. A record state transition outside
// free -> registered -> {delivered | cancelled} -> free,
...
// Scheduler destroyed with a non-quiescent wait registry — a
...
// (ADR-explicit-io-request-contract, Accepted, Decision 5): the
...
// (ADR Decision 15 / AC-13 :566-572): slot release via reset/destroy is
...
// (design §9 / ADR Decision 4 :341-349): enqueue from any slot state
...
// (ADR Decision 15 / AC-13): arena destruction with slot_in_use != 0 is
...
// Reap reached a backend_ready slot whose
...
// Record_terminal on a slot that is not a legal terminal
...
// Enqueue on a stale handle is a reuse-before-ack
// invariant violation, not a successful Scheme-B no-op. Masking it as
...
// A slot whose 64-bit generation reached UINT64_MAX
// cannot increment without wrapping (which would re-introduce the ABA the
// generation contract forbids absolutely). Fail fast on generation exhaustion
// instead of silently wrapping. Detected in BOTH Debug and Release;
// practically unreachable (~585 years at 1 release/ns).
[[noreturn]] void request_arena_generation_exhausted_fail_fast() noexcept {
    std::terminate();
}

// An out-of-range SlotIndex passed to an
...
// Record_terminal was given an unstored
...
// The dispatch path reached a slot that is neither
...
// The dispatch path (mark_running) was given a stale
...
// The ready-ring push invariants were violated
// (slot not backend_ready / no stored terminal /
...
// ThreadPoolBackend non-quiescent destruction. The caller destroyed
...
// UringAsyncBackend non-quiescent destruction. The caller destroyed
