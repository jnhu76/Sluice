...
void Scheduler::sem_acquire(WaitQueue& waiters, std::atomic<std::uint32_t>& available,
                            WaitNode& node) {
    ...
    Fiber* me = ws->current;
    {
        ...
        if (node.prev_ == nullptr && available.load(std::memory_order::acquire) > 0) {
...
void Scheduler::sem_acquire_until(WaitQueue& waiters, std::atomic<std::uint32_t>& available,
...
bool Scheduler::sem_release(WaitQueue& waiters, std::atomic<std::uint32_t>& available,
