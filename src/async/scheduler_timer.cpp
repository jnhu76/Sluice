...
bool Scheduler::expire_wait(WaitQueue& q, WaitNode& node) {
    ...
    LockGuard lk(global_mtx_);
    LockGuard qlk(q.mtx());
    if (!q.expire_locked(node)) return false;
    if (waiting_waitq_count_ > 0) --waiting_waitq_count_;
...
TimerRegistration* Scheduler::register_test_deadline_locked(WaitNode* node,
    ...
    if (clock_now_unlocked() >= deadline) return nullptr;
    if (q != nullptr) {
        LockGuard qlk(q->mtx());
