...
Scheduler::ConditionAdmitDisposition
Scheduler::condition_wait_admit_locked(WaitQueue& cond_waiters, WaitNode& cond_node,
                                       const WaitResume& resume, WaitQueue& mutex_waiters,
                                       Fiber*& owner, bool timed, deadline_t deadline)
    SLUICE_REQUIRES(global_mtx_) {
    TimerRegistration* reg = nullptr;
    {
        LockGuard qlk(cond_waiters.mtx());
        if (timed) {
            reg = prepare_ordinary_deadline_locked(&cond_node, &cond_waiters, deadline);
        }
...
WaitOutcome Scheduler::condition_wait_prepare(WaitQueue& cond_waiters, WaitNode& cond_node,
                          ...
                          "caller precondition violation");
    {
        ...
        const ConditionAdmitDisposition disp =
            condition_wait_admit_locked(cond_waiters, cond_node, WaitResume::fiber(me),
                                        mutex_waiters, owner, false, deadline_t{});
...
WaitOutcome Scheduler::condition_wait_prepare_until(WaitQueue& cond_waiters, WaitNode& cond_node,
                                                    WaitQueue& mutex_waiters, Fiber*& owner,
                          ...
                          "caller precondition violation");
    {
        ...
        const ConditionAdmitDisposition disp = condition_wait_admit_locked(
            cond_waiters, cond_node, WaitResume::fiber(me), mutex_waiters, owner, true, deadline);
        if (disp != ConditionAdmitDisposition::authorized &&
            disp != ConditionAdmitDisposition::resolved_inline_released) {
            released_mutex = false;
            return cond_node.outcome();
...
bool Scheduler::condition_cancel_wait(WaitQueue& cond_waiters, WaitNode& cond_node) {
    LockGuard lk(global_mtx_);
    LockGuard qlk(cond_waiters.mtx());
    if (!cancel_primitive_wait_locked(cond_waiters, cond_node))
        return false;
    if (waiting_waitq_count_ > 0)
        --waiting_waitq_count_;
