...
ApplicationRuntime* ApplicationRuntime::current_fiber_tag() noexcept {
    return static_cast<ApplicationRuntime*>(Scheduler::current_fiber_execution_tag());
}

CancelToken& RuntimeTaskContext::cancel_token() noexcept {
    return *token_;
}
...
Result<void> RuntimeTaskContext::await_completion(Completion<std::size_t>& c) {
    assert(!c.idle() && "await_completion requires a submitted or ready Completion "
...
Result<void> ApplicationRuntime::start() {
    ...
    runtime_cv_.wait(lk, [this] {
        return driver_state_ == DriverState::barrier_wait || driver_state_ == DriverState::exited;
    });
    ...
    if (test_pause_at_commit_checkpoint_.load(std::memory_order::acquire)) {
        ...
        runtime_cv_.notify_all();
        runtime_cv_.wait(lk,
                         [this] { return commit_release_flag_.load(std::memory_order::acquire); });
    }
#endif
    if (stop_requested_) {
        ...
        runtime_cv_.wait(lk, [this] { return driver_state_ == DriverState::exited; });
        lk.unlock();
        if (driver_thread_.joinable())
            driver_thread_.join();
        close_resources();
...
Result<void> ApplicationRuntime::drain() {
    ...
    runtime_cv_.wait(lk, [this] { return drain_complete_ || state_ == State::Fatal; });
...
Result<void> ApplicationRuntime::shutdown() {
    ...
    case State::Draining: {
        runtime_cv_.wait(lk, [this] {
            return drain_complete_ || state_ == State::Fatal || close_state_ == CloseState::Closed;
        });
        if (close_state_ == CloseState::Closed)
            return {};
        lk.unlock();
...
void ApplicationRuntime::driver_main() {
    ...
    runtime_cv_.wait(lk, [this] { return state_ != State::Starting || startup_abort_requested_; });
    ...
    for (;;) {
        ...
        if (task_set_terminal_snapshot_.load(std::memory_order::acquire) &&
            ...
            runtime_cv_.wait(lk, [this] {
                return driver_exit_requested_ || fatal_snapshot_.load(std::memory_order::acquire) ||
...
void ApplicationRuntime::test_dump_forensics(const char* tag) {
    static const char* kStateName[] = {"Constructed", "Starting", "Running",     "Stopping",
                                       "Draining",    "Stopped",  "StartFailed", "Fatal"};
    static const char* kDriverName[] = {"not_started",         "barrier_wait", "in_run_live",
                                        "between_invocations", "drained_wait", "exited"};
    State s;
    DriverState ds;
    ...
    std::fprintf(stderr,
                 ...
                 tag, kStateName[static_cast<int>(s)], kDriverName[static_cast<int>(ds)],
                 static_cast<unsigned long>(ctrl), static_cast<unsigned long>(obs), admitted,
                 terminal, (int)drain_done, (int)stop);
    if (io_ctx_) {
        std::fprintf(stderr, "[issue116-forensics] io %s: accepted_outstanding=%zu\n", tag,
                     io_ctx_->outstanding());
    }
    if (sched_) {
