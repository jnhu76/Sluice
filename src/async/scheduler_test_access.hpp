...
struct Scheduler::AsyncTestAccess {
    ...
    static bool worker_loop_exited(const Scheduler& s, unsigned worker_id) noexcept {
        ...
        return worker_id < s.workers_.size() &&
               s.workers_[worker_id]->loop_exited.load(std::memory_order_acquire);
    }
    static std::size_t worker_local_runnable(const Scheduler& s, unsigned worker_id) {
        LockGuard lk(s.global_mtx_);
    ...
    static WorkerState::ParkDomain worker_park_domain(const Scheduler& s,
                                                      unsigned worker_id) noexcept {
        LockGuard lk(s.global_mtx_);
        if (worker_id >= s.workers_.size())
            return WorkerState::ParkDomain::None;
        return s.workers_[worker_id]->park_domain.load(std::memory_order_acquire);
    }

    static WorkerState::ParkDomain
    worker_park_domain_try(const Scheduler& s, unsigned worker_id,
    ...
    static int worker_last_classify(const Scheduler& s, unsigned worker_id) noexcept {
        LockGuard lk(s.global_mtx_);
        return worker_id < s.workers_.size()
                   ? s.workers_[worker_id]->last_classify.load(std::memory_order_acquire)
                   : -1;
    }
    ...
    static TimerRegistration* register_test_deadline(Scheduler& s, WaitNode* node, WaitQueue* q,
                                                     deadline_t deadline);
    ...
    static void select_event_link(Scheduler& s, Event& event, detail::SelectArmSlot& arm) {
        LockGuard lk(s.global_mtx_);
        s.select_event_link_locked(event, arm);
    }

    static void select_event_unlink(Scheduler& s, Event& event, detail::SelectArmSlot& arm) {
        LockGuard lk(s.global_mtx_);
    ...
    static void set_arm_state(Scheduler& s, detail::SelectArmSlot& arm, detail::ArmState st);

    static void select_event_forge_stale_home(Scheduler& s, Event& event,
                                              detail::SelectArmSlot& arm);

    static void select_event_forge_wrong_home(Scheduler& s, Event& event_a, Event& event_b,
                                              detail::SelectArmSlot& arm);

    static detail::SelectTimerRegistration*
    register_synthetic_select_timer(Scheduler& s, detail::SelectArmSlot* arm, deadline_t deadline) {
    ...
    static bool retire_synthetic_select_timer(Scheduler& s, detail::SelectTimerRegistration& reg) {
        LockGuard lk(s.global_mtx_);
        return s.select_timer_retire_locked(reg);
    }
    static bool consume_synthetic_select_timer(Scheduler& s, detail::SelectTimerRegistration& reg) {
        LockGuard lk(s.global_mtx_);
        return s.select_timer_consume_locked(reg);
    }

    static detail::SelectTimerRegistration*
    splice_one_for_test(Scheduler& s, std::list<detail::SelectTimerRegistration>& tmp_pool,
    ...
    static bool detached_try_claim_expiry(detail::SelectTimerRegistration& reg) noexcept {
    ...
    static bool detached_retire(detail::SelectTimerRegistration& reg) noexcept {
        assert(reg.scheduler() == nullptr &&
               "detached CAS accessor requires a never-registered registration");
        if (reg.scheduler() != nullptr) {
            detail::select_invariant_fail_fast();
    ...
    static bool detached_claim_winner(detail::SelectGroup& group, std::uint32_t arm_index) noexcept;

    static bool select_process_group(Scheduler& s, detail::SelectGroup& group,
                                     std::uint32_t candidate_index);

    static bool select_all_authority_closed(const Scheduler& s, const detail::SelectGroup& group);

    static void assert_select_all_authority_closed(const Scheduler& s,
                                                   const detail::SelectGroup& group);

    static void advance_clock(Scheduler& s, deadline_t t);

    static std::size_t select_timer_pool_size(const Scheduler& s) noexcept;
    static std::size_t
    select_timer_count_in_state(const Scheduler& s,
                                detail::SelectTimerRegistration::State st) noexcept;

    static std::array<std::size_t, 2> tagged_heap_counts_by_kind(const Scheduler& s) noexcept;

    static bool
    deadline_heap_has_select_target(const Scheduler& s,
                                    const detail::SelectTimerRegistration* target) noexcept;

    static std::size_t select_timer_arm_load_count(const Scheduler& s) noexcept {
    ...
    static SelectResult group_result(const Scheduler& s, const detail::SelectGroup& group);
    ...
    static void select_begin_rollback(Scheduler& s, detail::SelectGroup& group) {
        LockGuard lk(s.global_mtx_);
    ...
    static void select_finish_rollback(Scheduler& s, detail::SelectGroup& group,
                                       detail::SelectArmSlot* arms, std::size_t arm_count,
                                       std::size_t registered_count) {
        LockGuard lk(s.global_mtx_);
        s.select_finish_rollback_locked(group, arms, arm_count, registered_count);
    }
    static void select_rollback_registration(Scheduler& s, detail::SelectGroup& group,
                                             detail::SelectArmSlot* arms, std::size_t arm_count,
                                             std::size_t registered_count) {
        LockGuard lk(s.global_mtx_);
        s.select_rollback_registration_locked(group, arms, arm_count, registered_count);
    }
    ...
    static bool select_resolve_timer(Scheduler& s, detail::SelectTimerRegistration& reg) {
        LockGuard lk(s.global_mtx_);
    ...
    static void drain_select_pool(Scheduler& s) {
        {
            ...
            for (auto& reg : s.select_timer_pool_) {
                if (reg.is_active())
                    s.select_timer_retire_locked(reg);
            }
        }
    ...
    static void rwlock_death_forge_invalid_head_mode(Scheduler& s, AsyncRwLock& rw);

    static void rwlock_death_forge_null_head_user(Scheduler& s, AsyncRwLock& rw);

    static void rwlock_death_forge_invalid_batch_member(Scheduler& s, AsyncRwLock& rw);
    ...
    static RunnablePublicationSnapshot capture_runnable_publication(Scheduler& s, Fiber& fiber,
                                                                    unsigned owner_worker_id,
                                                                    unsigned resolver_worker_id) {
        ...
        if (owner_worker_id < s.workers_.size()) {
            snap.owner_suspend_switch_pending =
                s.workers_[owner_worker_id]->suspend_switch_pending.load(std::memory_order_acquire);
        }
        ...
        if (owner_worker_id < s.workers_.size()) {
            ...
            for (auto* f : w.local_runnable) {
                if (f == &fiber) {
                    snap.owner_queue_contains_fiber = true;
                    break;
                }
            }
        }

        if (resolver_worker_id < s.workers_.size() && resolver_worker_id != owner_worker_id) {
            auto& w = *s.workers_[resolver_worker_id];
            std::lock_guard<std::mutex> wlk(w.inbox_mtx);
            for (auto* f : w.local_runnable) {
                if (f == &fiber) {
                    snap.resolver_queue_contains_fiber = true;
                    break;
                }
            }
        }
        ...
        for (auto* f : s.pending_spawn_) {
            if (f == &fiber) {
                snap.pending_spawn_contains_fiber = true;
                break;
            }
        }
        return snap;
    ...
    static bool event_wait_deferred_for_test(Scheduler& s, Event& event, WaitNode& node,
                                             FeDeferredRecord& record);

    static bool event_wait_deferred_deadline_for_test(Scheduler& s, Event& event, WaitNode& node,
                                                      FeDeferredRecord& record,
                                                      deadline_t deadline);

    static bool event_cancel_deferred_for_test(Scheduler& s, Event& event, WaitNode& node);

    static std::size_t take_deferred_for_test(Scheduler& s, void** out, std::size_t cap) {
    ...
    static bool queue_push_deferred_until_for_test(Scheduler& s, detail::QueuePort& port,
                                                   detail::QueueItemLease& lease, WaitNode& node,
                                                   QueueWaitCtx& ctx, FeDeferredRecord& record,
                                                   deadline_t deadline);
    static bool queue_pop_deferred_for_test(Scheduler& s, detail::QueuePort& port,
                                            detail::QueueItemLease& out, WaitNode& node,
                                            QueueWaitCtx& ctx, FeDeferredRecord& record);
    static bool queue_pop_deferred_until_for_test(Scheduler& s, detail::QueuePort& port,
                                                  detail::QueueItemLease& out, WaitNode& node,
                                                  QueueWaitCtx& ctx, FeDeferredRecord& record,
                                                  deadline_t deadline);

    static bool queue_cancel_deferred_for_test(Scheduler& s, detail::QueuePort& port,
                                               detail::QueueRole role, WaitNode& node) {
    ...
    static bool queue_push_core_(Scheduler& s, detail::QueuePort& port,
                                 detail::QueueItemLease& lease, WaitNode& node,
                                 FeDeferredRecord& record, bool timed, deadline_t deadline);
    static bool queue_pop_core_(Scheduler& s, detail::QueuePort& port, detail::QueueItemLease& out,
                                WaitNode& node, FeDeferredRecord& record, bool timed,
                                deadline_t deadline);

    static void queue_release_deferred_pin_for_test(detail::QueuePort& port);

    static std::size_t queue_active_port_calls_for_test(const detail::QueuePort& port) {
        LockGuard glk(port.scheduler_.global_mtx_);
        LockGuard slk(port.state_mtx_);
        return port.active_port_calls_;
    }

    static std::size_t queue_active_wait_associations_for_test(const detail::QueuePort& port) {
        LockGuard glk(port.scheduler_.global_mtx_);
        LockGuard slk(port.state_mtx_);
        return port.active_wait_associations_;
    }
    static std::size_t queue_active_queue_timers_for_test(const detail::QueuePort& port) {
        LockGuard glk(port.scheduler_.global_mtx_);
        LockGuard slk(port.state_mtx_);
        return port.active_queue_timers_;
    }
    static std::size_t queue_granted_not_resumed_for_test(const detail::QueuePort& port) {
        LockGuard glk(port.scheduler_.global_mtx_);
        LockGuard slk(port.state_mtx_);
        return port.granted_not_resumed_;
    }

    static bool rwlock_read_deferred_for_test(Scheduler& s, AsyncRwLock& lock, WaitNode& node,
                                              void* actor_token, RwWaitCtx& ctx,
                                              FeDeferredRecord& record);
    static bool rwlock_read_deferred_until_for_test(Scheduler& s, AsyncRwLock& lock, WaitNode& node,
                                                    void* actor_token, RwWaitCtx& ctx,
                                                    FeDeferredRecord& record, deadline_t deadline);
    static bool rwlock_write_deferred_for_test(Scheduler& s, AsyncRwLock& lock, WaitNode& node,
                                               void* actor_token, RwWaitCtx& ctx,
                                               FeDeferredRecord& record);
    static bool rwlock_write_deferred_until_for_test(Scheduler& s, AsyncRwLock& lock,
                                                     WaitNode& node, void* actor_token,
                                                     RwWaitCtx& ctx, FeDeferredRecord& record,
                                                     deadline_t deadline);

    static bool rwlock_try_write_deferred_for_test(Scheduler& s, AsyncRwLock& lock,
                                                   void* actor_token);

    static void rwlock_unlock_write_deferred_for_test(Scheduler& s, AsyncRwLock& lock,
                                                      void* actor_token);

    static bool rwlock_read_core_(Scheduler& s, AsyncRwLock& lock, WaitNode& node,
                                  void* actor_token, RwWaitCtx& ctx, FeDeferredRecord& record,
                                  bool timed, deadline_t deadline);
    static bool rwlock_write_core_(Scheduler& s, AsyncRwLock& lock, WaitNode& node,
                                   void* actor_token, RwWaitCtx& ctx, FeDeferredRecord& record,
                                   bool timed, deadline_t deadline);
    ...
    static bool rwlock_cancel_deferred_for_test(Scheduler& s, AsyncRwLock& lock, WaitNode& node);

    static bool rwlock_writer_active_for_test(AsyncRwLock& lock);
    static bool rwlock_owned_by_for_test(AsyncRwLock& lock, const void* actor_token);

    static bool condition_wait_deferred_for_test(Scheduler& s, WaitQueue& cond_waiters,
                                                 WaitNode& cond_node, WaitQueue& mutex_waiters,
                                                 Fiber*& owner, FeDeferredRecord& record,
                                                 bool& released);
    static bool condition_wait_deferred_until_for_test(Scheduler& s, WaitQueue& cond_waiters,
                                                       WaitNode& cond_node,
                                                       WaitQueue& mutex_waiters, Fiber*& owner,
                                                       deadline_t deadline,
                                                       FeDeferredRecord& record, bool& released);

    static void condition_notify_one_for_test(Scheduler& s, WaitQueue& cond_waiters) {
        s.condition_notify_one(cond_waiters);
    }
    static std::size_t condition_notify_all_for_test(Scheduler& s, WaitQueue& cond_waiters) {
        return s.condition_notify_all(cond_waiters);
    }
    static bool condition_cancel_for_test(Scheduler& s, WaitQueue& cond_waiters,
                                          WaitNode& cond_node) {
        return s.condition_cancel_wait(cond_waiters, cond_node);
    }

    static bool condition_wait_deferred_core_(Scheduler& s, WaitQueue& cond_waiters,
                                              WaitNode& cond_node, WaitQueue& mutex_waiters,
                                              Fiber*& owner, FeDeferredRecord& record, bool timed,
