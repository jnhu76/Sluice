...
Result<std::unique_ptr<Writer>> BlockingIoContext::open_writer(std::string_view path,
                                                               OpenWriterOptions options) {
    auto writer = std::make_unique<FileWriter>(std::string(path), options.syscall_stats,
                                               options.vector_stats, options.sync_stats);
    if (!writer->opened()) {
        return make_unexpected<std::unique_ptr<Writer>>(
            writer->open_error().value_or(IoError{.code = IoError::Code::permission_denied}));
    }
