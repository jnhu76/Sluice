...
class StrReader final : public cppio::Reader {
public:
    cppio::MemoryReader mem;
    explicit StrReader(std::string_view s) : mem(cppio::MemoryReader::from_string(s)) {}
    cppio::Result<std::size_t> read_some(std::span<std::byte> dst) override {
        return mem.read_some(dst);
    }
};

void run(const char* label, StrReader& reader, cppio::BufferedReader* buffered,
         cppio::Writer& writer, std::span<std::byte> scratch, cppio::CopyOptions opts) {
    cppio::CopyStats st;
    cppio::CopyDecision dec;
    cppio::Reader& src = buffered ? static_cast<cppio::Reader&>(*buffered)
                                  : static_cast<cppio::Reader&>(reader);
    auto res = cppio::copy_all(src, writer, scratch, opts, &st, &dec);
    std::printf("=== %s ===\n", label);
    if (!res.has_value()) {
    ...
    std::printf("  requested_strategy=%s\n", cppio::to_string(dec.requested).data());
    std::printf("  selected_strategy=%s\n", cppio::to_string(dec.selected).data());
...
int main() {
    ...
    {
        StrReader r(data);
        cppio::MemoryWriter w;
        cppio::CopyOptions o; o.strategy = cppio::CopyStrategy::Scratch;
    ...
    // 2. BufferedFirst strategy (prime a BufferedReader so fast path engages)
    {
        ...
        cppio::BufferedReader br(r, rbuf);
        std::vector<std::byte> primed(4);
        (void)br.read_some(primed);  // leave 12 buffered
        cppio::MemoryWriter w;
        cppio::CopyOptions o; o.strategy = cppio::CopyStrategy::BufferedFirst;
    ...
    // 3. Auto strategy (same setup; Auto resolves to BufferedFirst)
    {
        ...
        cppio::MemoryWriter w;
        cppio::CopyOptions o; o.strategy = cppio::CopyStrategy::Auto;
    ...
    // 4. Deferred strategy rejected (default policy)
    {
        StrReader r(data);
        cppio::MemoryWriter w;
        cppio::CopyOptions o; o.strategy = cppio::CopyStrategy::VectorDeferred;
    ...
    // 5. Deferred strategy fallback to Auto
    {
        ...
        cppio::MemoryWriter w;
        cppio::CopyOptions o;
        o.strategy = cppio::CopyStrategy::SpliceDeferred;
        o.unsupported_policy = cppio::UnsupportedStrategyPolicy::FallbackToAuto;
