...
\* docs/history/closeout/e12-async-mutex.md). This is the formal half of E12-C. The gate is
...
\*                                    ownership authority; no redundant locked_.
\*   M2  FIFO Handoff                unlock transfers ownership to the eligible
\*                                    FIFO head; no barging.
\*   M3  No Barging                  a newcomer cannot acquire while an eligible
\*                                    queued waiter has FIFO priority.
\*   M4  Owner-Before-Publication    on handoff, owner := winner BEFORE the
\*                                    winner is published runnable.
\*   M5  Grant/Publication Coupling  a handoff publication arises iff ownership
\*                                    committed to the winner.
\*   M6  Admission Closure           register + recheck + disposition is ONE
\*                                    atomic step; no Registered-but-not-
\*                                    Suspended queue member; no ownerless
\*                                    queued demand.
\*   M7  Deadline Precedence         immediately admissible ownership beats an
\*                                    already-due deadline (free+due -> Woken).
\*   M8  Grant Finality              a late cancel/expire after Woken cannot
\*                                    change owner or republish.
\*   M9  Publication Discipline      only UnlockHandoff / CancelSuspended /
\*                                    ExpireSuspended publish; immediate /
\*                                    admission-acquire / terminal-attempt /
\*                                    no-waiter / Destroy do NOT publish.
\*   M10 Destruction Precondition    Destroy only when owner = NoOwner /\ queue empty.
\*   M11 Single Resolution           each epoch resolves at most once.
\*   M12 Queue Well-Formedness       queue is duplicate-free; every queued epoch
\*                                    is Registered (= Suspended in production).
\*
\* SCOPE. This is a SAFETY-ONLY model.
\*   - Each admission action (LockAdmission / LockUntil family) refines the ENTIRE
\*     production register + recheck + (inline-Woken | inline-Expired |
\*     make_waiting) critical section as ONE atomic step. There is NO standalone
\*     registration action and NO admissionPhase field: a Registered epoch in the
\*     queue IS a Suspended (make_waiting-committed) epoch. The model MUST NOT
\*     expose a Registered-but-not-Suspension-committed queue state.
\*   - Each Epoch denotes ONE wait epoch (not a reusable production WaitNode
\*     across epochs). resolutionCount/publicationCount are bounded by 1.
\*   - deadlineDue[epoch] is the environment-chosen admission input (was the
\*     deadline already due at admission). Each timed admission action selects
\*     its branch and latches admissionSawFree/admissionSawDue atomically with
\*     the resolution so the history-backed invariants are prime-free.
\*   - Expired has full first-class status (LockUntilDue admission + ExpireSuspended
\*     + ExpireAttemptTerminal) so both halves of deadline precedence and grant
\*     finality are reachable and non-vacuous.
\*   - Under Conclusion A (E10/E12-B), a linked eligible FIFO head observed under
\*     the coordination lock cannot lose its resolve_(Woken) CAS; no negative
\*     model assumes a linked head loses.
\*
\* Refinement map (TLA+ concept -> production seam):
\*   owner                 <-> AsyncMutex::owner_ (Fiber, sole authority)
\*   queue                 <-> AsyncMutex::waiters_ (private WaitQueue intrusive FIFO)
\*   nodeState             <-> WaitNode::state_ (atomic CAS)
\*   epochFiber            <-> WaitNode::fiber() (captured at registration)
\*   TryLockSuccess        <-> Scheduler::mutex_try_lock (free + empty queue)
\*   LockImmediate         <-> mutex_lock admission: free + FIFO head -> inline Woken
\*   LockAdmissionAcquire  <-> mutex_lock admission recheck: speculatively registered,
\*                             recheck free + FIFO head -> inline Woken, unlink spec
\*   LockAdmissionSuspend  <-> mutex_lock admission: register + recheck not admissible
\*                             -> make_waiting (Registered in queue = Suspended)
\*   LockUntilImmediate    <-> mutex_lock_until admission: free + due -> inline Woken
\*   LockUntilAdmissionAcquire <-> mutex_lock_until admission recheck admissible
\*   LockUntilAdmissionSuspend <-> mutex_lock_until admission not admissible + future
\*   LockUntilDue          <-> mutex_lock_until admission not admissible + due -> Expired
\*   UnlockNoWaiter        <-> mutex_unlock empty-queue: owner := NoOwner
\*   UnlockHandoff         <-> MUTEX-HANDOFF-ONE: owner:=winner BEFORE publication
\*   CancelSuspended       <-> mutex_cancel: Registered queued -> Cancelled + publish
\*   ExpireSuspended       <-> timer expiry: Registered queued -> Expired + publish
\*   CancelAttemptTerminal <-> late cancel against terminal epoch (no-op loser)
\*   ExpireAttemptTerminal <-> late expire against terminal epoch (no-op loser)
\*   Destroy               <-> ~AsyncMutex precondition assert (owner=NoOwner, queue empty)
\*
\* History / ghost evidence (marked HISTORY below) exists ONLY so transition
\* properties (M2/M3/M4/M5/M7/M8/M10) are expressible as pure STATE invariants.
\* They are NOT production fields. preOwner is the PREVIOUS Mutex ownership,
\* NOT the acting Fiber.
