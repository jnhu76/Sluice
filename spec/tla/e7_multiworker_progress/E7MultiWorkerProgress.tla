...
(* MW-Inv8: no local inference of global MW-S2. Structural/commentary only:
   the classifier is global (MW_S1/S2 are defined over ALL workers' state, not
   any single worker's local queue). There is no per-worker "MW-S2" notion
   that could be inferred from an empty local queue. This is encoded
   structurally in MW_S1/MW_S2 (they quantify over all of Workers). The
   formula below is therefore a tautology and is NOT in the checked Inv set;
   it is kept as documentation of the "classifier is global" property. *)
(* MWInv8 is structural; not checked by TLC. *)
