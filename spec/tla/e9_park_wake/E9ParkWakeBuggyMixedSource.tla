-------------------- MODULE E9ParkWakeBuggyMixedSource --------------------
(*
...
EnterPhysicalPark(w) ==
    ...
    /\ IF /\ wakeEpoch = observedEpoch[w]   \* no pending SCHEDULER wake
       ...
       THEN
          ...
          \* BUGGY: ~ExternalWakePossible guard OMITTED. MW-S2 participant
          \* enters the BACKEND park domain even when external wake is
          \* possible (the blind mixed-source wait, E9 spec 14).
          /\ IF /\ MWS2
...
LeavePark(w) ==
    ...
    /\ (\/ wakeEpoch # observedEpoch[w]   \* SCHEDULER-domain wake (epoch advance)
        \/ backendReady                   \* BACKEND-domain wake (backend progress)
        \/ ExecutableWork                 \* work appeared (e.g. DrainExternalReady ran)
        \/ runnableVisible)               \* spurious / coalesced wake still re-drains
