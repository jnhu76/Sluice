#!/usr/bin/env bash
# verify-async-mutex.sh -- reproducible E12-C AsyncMutex TLA+ / TLC
...
#   TLA2TOOLS_JAR=/opt/tla2tools.jar scripts/formal/verify-async-mutex.sh
#
...
cleanup() {
  if [[ -n "$outroot" ]] && [[ "$outroot" == *sluice-formal.e12-mtx.* ]]; then
    rm -rf -- "$outroot"
  fi
}
trap cleanup EXIT
