...
#                                         GENERATED by _gen_neg.py, #192)
#
# Freshness prerequisite (#192): NegRetireDead and BuggyNoBridge are full
# duplicated snapshots, so they are GENERATED from the current E9ParkWake.tla
# by _gen_neg.py and byte-compared (--check) BEFORE any TLC run. A stale,
# missing, or unexpected generated artifact fails the gate without starting
# TLC (a stale negative keeps checking an outdated mutation while CI stays
# green). BuggyDrainParks (EXTENDS-based) auto-tracks the positive model;
# BuggyPrePark/BuggyMixedSource are FROZEN pre-Phase-G historical controls
# (hand-written, intentionally NOT generated).
#
