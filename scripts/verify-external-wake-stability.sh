...
#
# Thin wrapper over verify-runnable-steal-stability.sh (which is the canonical,
# generic, binary-agnostic reproducible runner for sluice stress gates). Selects ONE
...
#
# Usage / arguments / exit status: identical to verify-runnable-steal-stability.sh.
#   scripts/verify-external-wake-stability.sh MODE BINARY [FILTER] [COUNT]
set -euo pipefail

here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
exec "$here/verify-runnable-steal-stability.sh" "$@"
