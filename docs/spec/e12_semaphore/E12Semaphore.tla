------------------------------- MODULE E12Semaphore -------------------------------
(*
  sluice::async::Semaphore -- first-scope acquire-one / release-one async
  ...
  The load-bearing E12-B questions:
    P1  Permit Conservation       available + acquiredCount
                                       == initialPermits + acceptedReleaseCount
    P2  Release Disposition       every accepted release does exactly one of:
                                   transfer (acqRel+1, acquired+1, avail same)
                                   store     (acqRel+1, avail+1)
                                   reject    (no counter change)
    P3  FIFO Grant                a release transfers to the eligible FIFO head
    P4  Overflow Non-Mutation     release at available==max mutates nothing
    P5  No Idle Permit w/ Waiter  available>0 => no eligible queued waiter
    P6  Admission Closure         a wait admitted while available>0 (and no
                                   eligible waiter) resolves Woken inline
    P7  Permit-First Deadline     available>0 + deadline due => Woken (not Expired)
    P8  Queue Well-Formedness     queue is duplicate-free; only Registered
                                   admissible nodes are queued
    P9  Single Resolution         each wait epoch resolves at most once
    P10 Single Publication        one winning epoch -> at most one publication

  SCOPE. This is a SAFETY-ONLY model.
    - release is an ATOMIC external action (ReleaseTransfer / ReleaseStore /
      ReleaseOverflow). There is NO pending-release state, so no release
      liveness is asserted (any such property would be vacuous or would
      unjustifiably assume external future release).
    - Each modeled Node denotes ONE wait epoch, not a reusable production
      WaitNode across multiple epochs. AcquiredCount therefore equals the
      number of Woken nodes (InvGrantCommitCoupling).
    - Expired IS in the nodeState domain so the model can prove BOTH halves
      of deadline precedence (permit+due -> Woken; no-permit+due -> Expired).
  ...
  Refinement map (TLA+ concept -> production seam):
    ...
    queue                <-> WaitQueue intrusive FIFO list
    nodeState            <-> WaitNode::state_ (atomic CAS)
    AcquireImmediate     <-> admission: available>0 + no eligible waiter
                              (resolve_(Woken) inline via the admission seam)
    AcquireUntilImmediate <-> permit-first deadline admission (Woken)
    AcquireUntilDueNoPermit<-> no-permit + deadline-due admission (Expired)
    AcquireRegister      <-> register into the Semaphore wait queue
    AcquireSuspend       <-> the register/recheck/suspend-commit window
                              (rechecks available; consumes if a permit appears)
    ReleaseTransfer      <-> release() with an eligible FIFO head (wake seam)
    ReleaseStore         <-> release() with no eligible waiter, avail<max
    ReleaseOverflow      <-> release() at avail==max (rejected, no mutation)
    ResolveCancel        <-> Scheduler::cancel_wait (resolve_(Cancelled)+unlink)

  History / ghost evidence (marked HISTORY below) exist ONLY so transition
  properties (P2/P3/P4) are expressible as pure state invariants. They are
  NOT production fields.
*)
...
\* ReleaseOverflow: no eligible waiter AND available == max. REJECTED. No
\* authoritative state / counter / queue / node mutation. HISTORY fields
\* (lastAction / pre*) are updated SOLELY so InvOverflowNonMutation can verify
\* the non-mutation as a state predicate (see docs §8.3).
ReleaseOverflow ==
    /\ EligibleSet = {}
    /\ available = MaxPermits
    /\ lastGrantedNode' = NoNode
