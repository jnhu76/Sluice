...
(*
  ...
  The load-bearing E12-E Model-B questions (task §C3):
    B1  ClosedAbsorbing                   close is monotonic (Open->Closed only)
    B2  NoCommitAfterClose                no producer commits into the ring
                                            after close linearizes
    B3  CommittedBeforeCloseRemainsDrainable an item committed before close
                                            remains FIFO-consumable by consumers
    B4  FailedPushRetainsOriginalItem     a failed/closed push returns the
                                            ORIGINAL item (no copy, no alias,
                                            no default-construct)
    B5  ClosedEmptyConsumerTerminal       closed + empty => consumer receives
                                            Closed (no spurious item, no hang)
    B6  NoBufferedItemDiscardOnClose      close never discards buffered items
    B7  CloseProducerRaceLinearizable     close vs producer commit is
                                            serialized (G+S critical section)

  SCOPE. SAFETY-ONLY, matching the E12-B Semaphore precedent and Model A.
    - Adds queueState in {Open, Closed}. CL1 Open->Closed; CL2 idempotent.
    - Producer admission requires Open: a fast push at Closed returns the lease
      (PushClosed, P3) with the item moving producer_operation->detached; the
      failed result OWNS the exact original ItemId (ghost failedPushItem tracks
      this so B4 is a prime-free state invariant).
    - Consumers continue to drain buffered items after close; only at
      Closed+empty do they receive Closed (C2 fast / C6 blocked).
    - Blocked producers parked at close time are completed Closed (P7 +
      PUB-P-CLOSED); their item stays with the failed result (B4).
    - Close vs producer commit is serialized by modelling both as atomic
      steps that each re-check queueState under the (implicit) G+S critical
      section; the model's transition relation enforces exactly one linearizes
      first (B7).
    - Teardown, timer expiry, and external cancellation remain OUT OF SCOPE.

  Refinement map (TLA+ concept -> production seam), additions over Model A:
    queueState            <-> AsyncQueue closed_ (bool)
    CloseLinearize        <-> CL1 (Open->Closed under G+S)
    IdempotentClose       <-> CL2 (Closed->Closed, re-reconcile fixed point)
    PushClosed            <-> P3 (Closed -> detached, opaque failed result
                                  owns the exact lease)
    ProducerClosedCommit  <-> P7 then PUB-P-CLOSED (Closed+FIFO head)
    ConsumerClosedEmpty   <-> C2 (Closed+empty fast pop -> Closed)
    ConsumerClosedCommit  <-> C6 then PUB-C-CLOSED (Closed+empty+FIFO head)
    failedPushItem        <-> HISTORY: ghost record that the failed-push result
                              owns the same ItemId that was admitted

  History / ghost evidence (marked HISTORY below) exists ONLY so transition
  properties (B4/B7) are expressible as pure state invariants. They are NOT
  production fields.
*)
...
\* P4 Try-push would-block (Open, ring full OR older eligible producer linked).
\* The item is returned to the caller (Detached); the failed result owns the
\* exact original ItemId (failedPushItem records it). No ring entry.
PushWouldBlock(p, it) ==
    /\ prodState[p] = "Detached"
    /\ prodPhase[p] = "NoAdmission"
    /\ it \in Items
