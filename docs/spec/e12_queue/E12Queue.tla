...
(*
  ...
  "Current binding semantic decisions"; state machine §7):
    ...
    A3  UniqueRingItem           ring ItemIds duplicate-free; one lease per slot
    A4  NoLostItem               every admitted item is ring / operation /
                                 failed-result / released -- never lost
    A5  NoDuplicatedItem         no ItemId in two places at once
    A6  FIFOBufferOrder          ring preserves insertion order
    A7  ProducerWaiterFIFO       producer grants go to the FIFO producer-waiter head
    A8  ConsumerWaiterFIFO       consumer grants go to the FIFO consumer-waiter head
    A9  NoBarging                newcomer cannot fast-commit/pop over older eligible
    A10 CommittedBeforePublished completion finalized BEFORE wakeDispatched++
    A11 NoPublishedPendingCompletion no publication without finalized completion
    A12 LocationConsistency      lease location matches the control-location table

  SCOPE. This is a SAFETY-ONLY model, matching the E12-B Semaphore precedent.
    - Every modeled PNode / CNode denotes ONE wait epoch (one-shot), not a
      reusable production WaitNode across epochs. This is the Corrective-2
      one-shot lease abstraction.
    - Every live ItemId has exactly one control location (Detached /
      ProducerOp / Ring / ConsumerOp / Released); move empties source and
      requires empty destination.
    - Producer and consumer waiters are kept in SEPARATE FIFO sequences
      (prodWaiters / consWaiters). No direct producer->consumer transfer: the
      payload always goes producer_operation -> ring -> consumer_operation.
    - No barging: a fast Push is refused while an eligible producer waiter is
      linked (FastPushAdmissible requires ProdEligibleSet = {}); a fast Pop is
      refused while an eligible consumer waiter is linked.
    - Reconciliation grants ONLY the FIFO head of the eligible role set
      (ProdFIFOHead / ConsFIFOHead), and finalizes the winner's completion
      BEFORE incrementing wakeDispatched (winner-before-publication).
    - External release/pop timing, timer expiry, cancellation, teardown, and
      close are OUT OF SCOPE here. Liveness would require unjustified external
      fairness; the model is safety-only.

  Refinement map (TLA+ concept -> production seam):
    Capacity              <-> AsyncQueue capacity_ (runtime fixed >= 1)
    ring                  <-> the FIFO lease ring (Seq of unique ItemId)
    itemLoc               <-> QueueItemControl::Location
    prodWaiters           <-> producer WaitQueue intrusive FIFO
    consWaiters           <-> consumer WaitQueue intrusive FIFO
    FastPushCommit        <-> P2 (Open+space+no older producer -> ring)
    PushWouldBlock        <-> P4 (full / older producer -> detached return)
    ProducerWaitRegister  <-> P5 (link Registered+Pending)
    ProducerGrantCommit   <-> P6 then PUB-P-COMM (reconciler, FIFO head)
    FastPopCommit         <-> C1 (ring nonempty + no older consumer)
    PopWouldBlock         <-> C3 (empty / older consumer)
    ConsumerWaitRegister  <-> C4 (link Registered+Pending)
    ConsumerGrantCommit   <-> C5 then PUB-C-COMM (reconciler, FIFO head)
    ReleaseItem           <-> C9 (consumer_operation -> released)
    wakeDispatched        <-> runnable publication count (PUB-P/C-COMM)

  History / ghost evidence (marked HISTORY below) exists ONLY so transition
  properties (A7/A8/A10/A11) are expressible as pure state invariants. They
  are NOT production fields.
*)
...
\* P4 Try-push would-block (ring full OR an older eligible producer is linked):
\* the producer's item stays in producer_operation then returns detached to the
\* caller. The producer's lease is NOT consumed (no ring entry). We model only
\* the would-block outcome here; the producer may then choose to register.
PushWouldBlock(p, it) ==
    /\ prodState[p] = "Detached"
    /\ prodPhase[p] = "NoAdmission"
    /\ it \in Items
