------------------------------- MODULE E7MultiWorkerProgressBuggyOutstanding -------------------------------
(*
...
\* THE DEFECT (§9): MW_S2 is derived from WAIT REGISTRATIONS instead of
\* authoritative backendOutstanding. A fiber waiting on a ready-flag (which
\* never produces a backend op) falsely makes the global state MW-S2,
\* enabling a spurious blocking admission that blocks forever (no backend op
\* to reap). The correct protocol derives MW-S2 from backendOutstanding only.
MW_S2 == /\ ~MW_S1
         /\ AnyWaitRegistration
MW_S3 == /\ ~MW_S1
         /\ ~backendOutstanding
...
\* The invariant the buggy model is expected to VIOLATE: MW_S2 must imply
\* authoritative backendOutstanding. With the buggy (wait-reg-derived)
\* classifier, a ready-flag waiter produces MW_S2 with backendOutstanding=FALSE.
InvMWS2ImpliesOutstanding ==
...
\* In the buggy model, check the property that must FAIL: blocking admission
\* must only happen when backendOutstanding was authoritative. We check that
\* a committed/blocking worker implies MW_S2 held with authoritative outstanding
\* at commit. Since the buggy classifier admits false MW-S2, blocking can occur
\* without backendOutstanding.
InvBlockingRequiresOutstanding ==
    blockingWorker # NONE => backendOutstanding
=====
