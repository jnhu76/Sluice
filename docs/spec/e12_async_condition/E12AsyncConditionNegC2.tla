...
\* NEG-C2: CancelNonRegistered — removes both conditionNodeState = "Registered" AND
\* InConditionQueue guards from CancelCondition, allowing cancel of any epoch
\* (including non-Registered, non-queued ones like Detached epochs in Init).
\* Expected violated property: InvConditionResolvedFinality.
CancelCondition(epoch) ==
    /\ ~destroyed
    \* NEG: no conditionNodeState/InConditionQueue guard — fires on ANY epoch.
