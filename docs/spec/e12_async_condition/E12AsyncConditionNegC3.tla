...
\* NEG-C3: ReturnOwnedNoGrant — does NOT give mutex to the returning fiber,
\* violating C-H1 (the caller must own the Mutex after wait returns).
\* Expected violated property: InvReturnedOwnsMutex.
ReturnOwned(actor) ==
    /\ ~destroyed
    /\ conditionReason[actor] \in {"Woken", "Cancelled", "Expired"}
    /\ waitPhase[actor] \in {"ConditionResolved", "ReacquirePending"}
    /\ waitPhase' = [waitPhase EXCEPT ![actor] = "Returned"]
    \* NEG: No mutexOwner' = actor (fiber does NOT own Mutex after return).
