...
\* NEG-C6: MutexUnlockHandoff corrupts expectedFIFOHead' = R2 (wrong epoch) while
\* granting to the real FIFO head. The invariant checks
\* lastGrantedEpoch = expectedFIFOHead, which fails since they differ.
...
MutexUnlockHandoff(actor) ==
    ...
    /\ LET w == MutexFIFOHead IN
       ...
       /\ mutexNodeState' = [mutexNodeState EXCEPT ![w] = "Woken"]
       /\ mutexResolutionCount' = [mutexResolutionCount EXCEPT ![w] = 1]
       \* NEG: expectedFIFOHead is set to R2 (a different epoch), not w.
       /\ expectedFIFOHead' = R2
