------------------------------- MODULE E12AsyncConditionNegC10 -------------------------------
...
EligibleMutexQueue ==
    SelectSeq(mutexQueue, LAMBDA m : mutexNodeState[m] = "Registered")
...
\* NEG-C10: reacquire-only mutex queue (skips OrdinaryEpochs, breaks unified FIFO).
EligibleMutexQueueReacquire ==
    SelectSeq(mutexQueue, LAMBDA m :
        mutexNodeState[m] = "Registered" /\ m \in ReacquireEpochs)
MutexFIFOHeadReacquire ==
    IF EligibleMutexQueueReacquire = <<>> THEN None
    ELSE Head(EligibleMutexQueueReacquire)
...
\* NEG-C10: SeparateQueues — MutexUnlockHandoff only grants to ReacquireEpochs,
\* skipping OrdinaryEpochs. Breaks unified FIFO (C-H8).
\* Expected violated property: InvOrdinaryAndReacquireFIFO.
MutexUnlockHandoff(actor) ==
    /\ ~destroyed
    /\ mutexOwner = actor
    \* NEG: handoff to reacquire-only FIFO head (skips OrdinaryEpochs, C-H8).
    /\ MutexFIFOHeadReacquire # None
    /\ LET w == MutexFIFOHeadReacquire IN
