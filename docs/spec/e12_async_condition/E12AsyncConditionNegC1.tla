...
\* NEG-C1: NonOwnerWait — removes the mutexOwner = actor precondition AND makes
\* the actor KEEP the mutex after entering ConditionWaiting (breaking C-H3).
\* Expected violated property: InvConditionWaiterDoesNotOwnMutex.
WaitAdmissionSuspend(actor) ==
    ...
    /\ LET c == condEpoch(actor) IN
       ...
       /\ registrationCommitted' = [registrationCommitted EXCEPT ![c] = TRUE]
       \* NEG: actor KEEPS the mutex while entering ConditionWaiting.
       /\ mutexOwner' = actor
