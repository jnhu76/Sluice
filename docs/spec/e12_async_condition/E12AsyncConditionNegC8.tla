...
\* NEG-C8: WaitReleaseBeforeRegister — the mutex is released but the Condition
\* node is NEVER registered (lost-notify window).
\* Expected violated property: InvNoLostNotifyWindow (registrationCommitted FALSE).
WaitAdmissionSuspend(actor) ==
    ...
    /\ LET c == condEpoch(actor) IN
       /\ conditionNodeState[c] = "Detached"
       \* NEG: mutex released but Condition node NOT registered.
       /\ IF EligibleMutexQueue = <<>>
          THEN /\ mutexOwner' = NoOwner
               /\ UNCHANGED <<mutexNodeState, mutexQueue,
                              mutexResolutionCount, mutexPublicationCount>>
          ELSE LET w == MutexFIFOHead IN
       ...
       \* NEG: NO condition registration — registrationCommitted stays FALSE.
       /\ conditionNodeState' = conditionNodeState
       /\ conditionQueue' = conditionQueue
       /\ registrationCommitted' = registrationCommitted
       \* Commit ConditionWaiting phase.
