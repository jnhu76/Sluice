...
\* NEG-C9: HandoffNonFIFO — always grants to epoch R1 (regardless of FIFO order),
\* so other eligible waiters are skipped. This breaks C-H8.
\* Expected violated property: InvEligiblePreMutexQueue.
MutexUnlockHandoff(actor) ==
    /\ ~destroyed
    /\ mutexOwner = actor
    \* NEG: handoff targets R1 specifically, not the FIFO head.
    /\ mutexNodeState[R1] = "Registered"
    /\ LET w == R1 IN
       /\ mutexNodeState' = [mutexNodeState EXCEPT ![w] = "Woken"]
       /\ mutexResolutionCount' = [mutexResolutionCount EXCEPT ![w] = 1]
       /\ expectedFIFOHead' = w
