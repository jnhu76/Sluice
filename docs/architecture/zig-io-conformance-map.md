...
| `Future` / `async` / `await` / `cancel` | Generic async computation | **Public `Future<T>`** (caller-owned value channel: producer `complete_with`, idempotent `await()`/`cancel()`, `cancel_token()` cooperative cancel) + `Group::async` task spawn + `RuntimeTaskContext::submit_* + await_completion` (op-level await); the physical wait delegates to a `WaitPolicy&` (Threaded = block thread, Evented = Fiber suspend via Scheduler) | F | I | `future.hpp` / `future_test.cpp` (public, sluice-CORE-028 T2); `group.hpp:58`; `wait_policy.hpp`; `fiber.hpp:60`; `scheduler.hpp:270`; `application_runtime.hpp:60-86` |
...
| `Select` | Multi-source exactly-one winner | `select()` over **Event + Timer cases only** — no I/O-Completion case (intentional defer — [decision](../design/selectable-async-composability-decision.md)) | P | I | `select.hpp:40-43`; `select_fwd.hpp:34-36`; design issue [#99](https://github.com/jnhu76/Sluice/issues/99) |
...
| clock / `now` / `sleep` / `Timeout` | Scheduler-integrated time | Monotonic `deadline_t`, `advance_clock`, `*_until` waits, `TimerRegistration`, `TimerSelectCase`; **no standalone public `sleep_for`/`Timeout` type** | F | I | `scheduler.hpp:451-463`; `timer_registration.hpp` |
