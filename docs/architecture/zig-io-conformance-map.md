...
| `Future` / `async` / `await` / `cancel` | Generic async computation | Fiber + Scheduler + `Group::async` + `await_completion`; **no public Future type** (Completion is the caller-owned result object) | F | I | `fiber.hpp:60`; `scheduler.hpp:272` |
...
| `CancelProtection` / `recancel` / `swapCancelProtection` / `checkCancel` | Delivery-blocking protection regions + re-arm | `CancelProtection` enum, `CancelState::swap_protection`, `CancelGuard` RAII, `CancelToken::rearm` (= Zig `recancel`), `check_cancel` | F | F | `cancel.hpp:37-147`; DIV-11 Resolved |
