...
|---|---|---|
| arena capacity bounded; full -> would_block; Completion idle (I3/I8) | `request_arena_test.cpp :: arena_capacity_bounded` | PASS (commit 1) |
| generation advances on release; stale key rejected (I6) | `request_arena_test.cpp :: arena_generation_advances_on_release`, `:: arena_stale_key_rejected` | PASS (commit 1) |
| slot_in_use vs accepted_outstanding are distinct counters | `request_arena_test.cpp :: arena_accounting_tracks_slot_in_use_vs_accepted_outstanding` | PASS (commit 1) |
| accepted terminal path allocation-independent (I9) | `request_arena_test.cpp :: arena_no_post_accept_allocation` | PASS (commit 1) |
| **explicit metric vocabulary (P1-05)**: `queue_full_retries` counts ONLY capacity pressure (would_block); caller lifecycle violations (invalid_state) count `invalid_state_rejections` — the two are never conflated | `async_completion_test.cpp :: async_stats_increment_on_submit_poll_wait` (invalid_state -> invalid_state_rejections, queue_full_retries stays 0), `reference_backend_no_alloc_test.cpp :: sync_full_arena_rejection_is_allocation_free` (would_block -> queue_full_retries) | PASS (round-2 review-fix: `AsyncStats::invalid_state_rejections` added; tally_submit split; the SQE-pressure case was renamed to `uring_sqe_pressure_retains_accepted_work` and no longer claims a queue_full_retries assertion) |
...
| **round-5 fix 1 — stale `mark_running` is a dispatch-identity invariant, not a backoff**: `mark_running`'s `false` is RESERVED for the legitimate dispatch backoff (a current-generation slot already `backend_ready` because a terminal winner won before dispatch). A handle `validate_` rejects (stale generation / free / out-of-range / wrong-provenance) is now `request_arena_dispatch_stale_fail_fast` in BOTH Debug and Release rather than masquerading as the backoff. The control case (current-generation `backend_ready` → `false`, no fail-fast) is `request_arena_cancel_intent_test.cpp :: mark_running_backs_off_on_backend_ready`. | `request_arena_death_test.cpp :: mark-running-stale-handle` (death — reserve→prepare→install-binding→commit→enqueue→record-terminal→reap→release advances generation, then `mark_running` on the STALE handle fails fast exit 86) | PASS (round-5: Debug + Release + ASan/UBSan + TSan death case exits 86) |
| **round-5 fix 2 — ready-ring tail membership is covered**: the ring is an intrusive singly-linked FIFO whose tail node legitimately carries `ready_next_ == kNotOnReadyRing` (the list terminator), so the link-state check alone could not distinguish "not on ring" from "linked as tail." `push_ready_locked_` now also rejects a slot that IS the current tail (`ready_count_ != 0 && ready_tail_ == idx`) and verifies the head/tail/count triple is structurally consistent (O(1); no list traversal, no allocation). The new tail/consistency clauses and the existing clauses all route through `request_arena_ready_ring_invariant_fail_fast`. The normal terminal-winner path already blocks double-push via `terminal_.stored`, so this is an inspection-verified invariant (the only push sites — `record_terminal` / pending-or-enqueued `cancel` — guarantee the preconditions), like the generation-exhaustion guard. | ready-ring guard is inspection-verified + exercised by every Scheme-B/reap case (`request_lifecycle_scheme_b_test`, `backend_scheme_b_race_test`, `reference_backend_arena_lifecycle_test`) under Debug/Release/ASan-UBSan/TSan with no spurious fail-fast | PASS (round-5: inspection-backed; no spurious fail-fast across all reap paths) |
...
| Gate | Command | Result |
|---|---|---|
| Clang Debug | `xmake f -m debug --toolchain=clang -y && xmake build sluice_core && xmake build sluice_async && xmake build -g test && xmake test -v` | PASS (round-5: 135/135 passed, 0 failed — adds `mark-running-stale-handle` death case; round-4: 135/135) |
| Clang Release (§6.1) | `xmake f -m release --toolchain=clang -y && ...` | PASS (round-5: 135/135 passed, 0 failed — the unstored-terminal / dispatch-state / dispatch-stale / ready-ring fail-fasts fire in BOTH modes; **round-5 also diagnosed+fixed a pre-existing Release-only `async_mutex_primitive_test` hang** (`mtx_t5_owner_before_publication_phase` coordinator used a bounded yield-loop that could exhaust before the phase paused → CHECK returned without `release_handoff` → worker blocked in `test_phase` forever; reproduced at baseline `b157fcb` ~75%; now uses `wait_handoff_paused`, 20/20 Release-clean); round-4: 135/135) |
| ASan + UBSan (§6.2) | `xmake f -m asanubsan --toolchain=clang -y && ...` | PASS (round-5: 135/135 passed, 0 sanitizer warnings — the dispatch-stale fail-fast and the tail-covered ready-ring guard are exercised clean; the capturing-binding + arena-direct cancel-intent path is clean; round-4: 135/135) |
| TSan (§6.3) — target 0 data races | `xmake f -m tsan --toolchain=clang -y && ...` | PASS (round-5: 135/135 passed, 0 data races — including the genuine two-thread `concurrent_submit_cancel_enqueue` case, the backend-level `backend_scheme_b_race_test`, and the new `mark-running-stale-handle` death case; round-4: 135/135; the allocation probe is compiled out under TSan because the TSan C++ runtime owns the new/delete symbols — the lifecycle/zero-side-effect assertions remain active) |
| negative-compile | `scripts/verify-completion-authority-negative-compile.sh` (12/12) + `scripts/verify-request-arena-negative-compile.sh` (6/6) | PASS (round-5: 18/18 — unchanged; round-4; round-2: NEG_SLOT_BINDING_PRIVATE added) |
| doc-check | `python3 scripts/check-doc-links.py --self-test && python3 scripts/check-doc-links.py && python3 scripts/verify-architecture-docs.py` | PASS (round-5: divergence-registry DIV-02 summary row corrected + duplicate `---` removed + design-doc I6 generation-exhaustion text corrected; round-4: DIV-14 + ADR round-4 closeout; round-2) |
...
- [x] Gate 4 evidence filled with ACTUAL results (every row PASS — round-5 backfilled the four "PENDING — commit 1" capacity rows with their committed-test results)
- [x] Conformance map updated (DIV-02 activated; zig-map rows advanced — commit 6)
- [x] Divergence registry updated (DIV-02 — commit 0; DIV-02 summary row corrected to its activated Phase B status in round 5)
- [x] Constitution rules satisfied (design §19)
- [x] AGENTS.md change-class gates run (Debug 135/135, Release 135/135 — round-5 also diagnosed and fixed a pre-existing Release-only `async_mutex_primitive_test` hang at `mtx_t5_owner_before_publication_phase` whose coordinator used a bounded yield-loop that could exhaust before the phase paused, leaving the worker blocked in `test_phase`; now uses `wait_handoff_paused`, 20/20 Release-clean; ASan/UBSan 135/135 clean, TSan 135/135 0 data races, both negative-compile gates 18/18, doc-check PASS — round-2/round-3/round-4/round-5)
