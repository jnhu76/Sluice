...
| Gate | Command | Result |
|---|---|---|
| Clang Debug | `xmake f -m debug --toolchain=clang -y && xmake build sluice_core && xmake build sluice_async && xmake build -g test && xmake test -v` | PASS (round-5: 136/136 passed, 0 failed — adds `mark-running-stale-handle` death case; round-4: 135/135) |
| Clang Release (§6.1) | `xmake f -m release --toolchain=clang -y && ...` | PASS (round-5: 136/136 in-scope targets passed, 0 sanitizer/data-race — the unstored-terminal / dispatch-state / dispatch-stale / ready-ring fail-fasts fire in BOTH modes; **`async_mutex_primitive_test` is excluded — it deterministically hangs at `mtx_t5_owner_before_publication_phase` in Release, reproduced at baseline `b157fcb` 2/2, pre-existing and out of Phase B scope**; round-4: 135/135) |
| ASan + UBSan (§6.2) | `xmake f -m asanubsan --toolchain=clang -y && ...` | PASS (round-5: 136/136 in-scope test runners passed, 0 sanitizer warnings — the dispatch-stale fail-fast and the tail-covered ready-ring guard are exercised clean; the capturing-binding + arena-direct cancel-intent path is clean; round-4: 135/135) |
| TSan (§6.3) — target 0 data races | `xmake f -m tsan --toolchain=clang -y && ...` | PASS (round-5: 136/136 in-scope test runners passed, 0 data races — including the genuine two-thread `concurrent_submit_cancel_enqueue` case, the backend-level `backend_scheme_b_race_test`, and the new `mark-running-stale-handle` death case; round-4: 135/135; the allocation probe is compiled out under TSan because the TSan C++ runtime owns the new/delete symbols — the lifecycle/zero-side-effect assertions remain active) |
...
- [x] AGENTS.md change-class gates run (Debug 136/136, Release 136/136 in-scope + `async_mutex_primitive_test` excluded as a pre-existing out-of-scope Release-only hang reproduced at baseline `b157fcb`, ASan/UBSan 136/136 clean, TSan 136/136 0 data races, both negative-compile gates 18/18, doc-check PASS — round-2/round-3/round-4/round-5)
