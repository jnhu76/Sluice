...
| `test:default-gate-targets` | 182 | (168 at issue-116 closeout + 12 file-tools application-track tests on 2026-08-18: sluice-copy safe-output, 4x sluice-hash, 4x sluice-grep, 3x sluice-tail + 1 performance-attribution `sluice_grep_matcher_differential_test`; live cross-reference — this row tracks the current mechanical count, see `scripts/gates/mechanical-facts.py`; 181 → 182 on 2026-08-19, the #135 Case B round added `failure_model_high_risk_test`) |
