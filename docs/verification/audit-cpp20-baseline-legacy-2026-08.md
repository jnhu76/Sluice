...
- Every script that bypasses xmake and invokes a compiler directly passes
  ...
  - `scripts/formal/*.sh` (4 scripts: verify-async-mutex.sh, verify-async-condition.sh, verify-async-semaphore.sh, verify-event.sh; `-std=c++20 -fsyntax-only`).
...
| F2 | P2 | README, docs/, CI | Compiler floor is documented (this PR adds the README section: GCC ≥ 11 / Clang ≥ 14) but **not frozen or CI-enforced**: CI proves only "current ubuntu-latest Clang builds", there is no GCC merge job and no floor job, so a floor regression has no gate until a nightly gcc-11 / clang-14 job exists. | `.github/workflows/ci.yml` (single job); README floor section (this PR) | Keep the GCC ≥ 11 / Clang ≥ 14 recommendation; freeze it only after the nightly floor job enforces it (issue #103) |
| F3 | P2 | `xmake.lua:22`, `src/async/scheduler.cpp:4428`, `fuzz/copy_all_fault_fuzz.cpp:160` | `-Wextra` not enabled (`set_warnings("all")` maps to `-Wall` only, as-built). Full-tree `-Wextra` probe flagged 4 TUs: **2 production/harness unused-parameter instances** — `scheduler.cpp` unused `wh` (FIXED) and fuzz harness unused `result` (FIXED) — plus **2 authority-probe TUs** (P3: the condition probe has an unused parameter; the mutex probe fails only by its intended sealed-name diagnostics, no unused parameter). | `-Wextra` scan of every TU under src/, tests/, examples/, bench/, fuzz/, apps/ | Enable `-Wextra` after the 2 probe `(void)` casts; the 2 production instances are already fixed |
...
| F5 | P3 | docs review history | Some review docs cite `-Wall -Wextra -pedantic` manual compile evidence while the build only emits `-Wall -Werror`. Cosmetic mismatch between recorded evidence flags and as-built flags. | `docs/history/reviews/E12-E-QUEUE-CORRECTIVE-2-INDEPENDENT-ADVERSARIAL-REVIEW-1.md` | No code action; noted for future evidence templates |
...
| `SchedulerWakeHandle::Control::mtx` as mutex-serialized callback lease (`scheduler.cpp:61-96,201-237`) | Explicitly NOT shared ownership/refcount; has a TLA+ proof (`spec/tla/e9_wake_handle_lifetime/`) |
...
| Real liburing 2.14 (xrepo) | clean configure from stub mode: `xmake f -c -m debug --toolchain=clang --with-liburing=true -y` (`-c` avoids the F6 stale stub-object hazard); rebuild: `xmake build sluice_async` + `xmake build sluice_experimental_uring` + `xmake build -g test`; run each of the 8 uring targets directly: `./build/linux/x86_64/debug/uring_d2_failure_noalloc_test`, `uring_backend_c2b_identity_test`, `uring_backend_c2c_waiter_borrow_test`, `uring_backend_c2e_close_drain_test`, `uring_submit_failure_test`, `uring_write_batch_test`, `uring_io_context_test`, `uring_stats_test` | all 8 PASS with `[evidence-meta] mode=real` (c2e `backend.available()` true) |
...
| `-Wextra` probe | every TU under src/, tests/, examples/, bench/, fuzz/, apps/ | 4 TUs flagged: 2 production/harness unused-parameter instances (both FIXED) + 2 authority-probe TUs (P3) |
