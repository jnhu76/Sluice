...
1. apply ONE single-point mutation to the production logic
   (`/tmp/c2c_mutate.py`, exact string replacement — the mutations are listed
   in the matrix below);
...
4. record the expected case, the actual failing case, the command, and the
   exit code;
5. restore the file from the pre-mutation snapshot (pristine git + the C2c
   guarded seams);
6. after all mutants, confirm `grep -c MUTANT include/` is zero and `git diff
   include/ src/` shows only the guarded test seams.
...
| F | lease dropped: reap closes registration but does not move the lease into the ReadyEvent | `reap()`: `if (false && s.waiter_delivery_present_)` (delivery block disabled) | `arena_lease_transfer_chain_reap_path` | same — the case's `sink.lease_ids.size() == 1` / lease-id check fires; skipped cleanup trips the destructor fail-fast | `SLUICE_TEST_FILTER=arena_lease_transfer_chain_reap_path xmake run request_waiter_borrow_lease_test` | 255 |
| G | registration allowed after terminal (state-matrix violation) | `register_waiter()`: delete the `state != pending/enqueued -> invalid_state` guard | `arena_waiter_registration_state_matrix` | same — registration succeeds on reserved/prepared/backend_ready; the case's per-state `invalid_state` checks fire; skipped cleanup trips the destructor fail-fast | `SLUICE_TEST_FILTER=arena_waiter_registration_state_matrix xmake run request_waiter_borrow_lease_test` | 255 |
...
After mutant H was restored (the last mutant applied):

- `grep -c "MUTANT" include/sluice/async/detail/request_arena.hpp` → 0 (no
  mutation marker remains);
- `git diff --stat include/ src/` → only the four guarded seam headers
  (`request_arena.hpp` +54, `reference_ready_sink.hpp` +22,
  `fake_backend.hpp` +86, `threadpool_backend.hpp` +88); no production logic
  change;
