...
**Scope:** real liburing, Clang Debug, branch
`test/phase-d2-uring-failure-noalloc`, baseline
...
| M8 | explicitly call `io_uring_submit` from poisoned wait before the `to_submit=0` enter | `SLUICE_TEST_FILTER=uring_d2_poison_wait_never_submits_quarantined_write xmake run uring_d2_failure_noalloc_test` | exit 1; deterministic post-wait drain observed `disk_bytes != 0` at line 560 | PASS |
...
path has entered `wait_cqe_without_submit()`. It then drains additional CQEs before checking the
file side effect. With an explicit submitting-wait mutant, the quarantined write deterministically
executes and the detector goes RED; with production `to_submit=0`, the file remains empty and the
detector is GREEN. The hook branch and type member are absent from production builds.
