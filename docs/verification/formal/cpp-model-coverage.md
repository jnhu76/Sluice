...
| audit #162 MODEL-007(b–e) unmodeled mechanisms (item **(a) is now modeled**: the I47-F2 `suspend_switch_pending` steal-refusal window — the transient E7/E8's atomic `SuspendFiber` collapses — has its own focused suite `spec/tla/e8_suspend_switch/`, issue #172, safety PASS + 3 exact negatives + R1/R2/R3 witnesses): Phase-F1 WaitRecord registry generation/lease/delivered races (`scheduler.cpp:1367-1419`); CancelToken task-cancel epoch protocol; #115 spawn-to-busy-worker wake-epoch obligation; G1 worker retire-ring ticket rescue (local_runnable→pending_spawn_) — umbrella tracker: issue #171 | any change to the corresponding mechanism, or issue #162 §7.3's second suggested focused model (WaitRecord registry Race B/C) being taken up |
