...
| audit #162 MODEL-007(a–e) unmodeled mechanisms: I47-F2 `suspend_switch_pending` steal-refusal window (E7/E8 atomic `SuspendFiber` collapses register+suspend+switch); Phase-F1 WaitRecord registry generation/lease/delivered races (`scheduler.cpp:1367-1419`); CancelToken task-cancel epoch protocol; #115 spawn-to-busy-worker wake-epoch obligation; G1 worker retire-ring ticket rescue (local_runnable→pending_spawn_) — umbrella tracker: issue #171 | any change to the corresponding mechanism, or issue #162 §7.3's two suggested focused models (suspend-switch vs steal window; WaitRecord registry Race B/C) being taken up — deliberately not auto-built (audit scope decision, recorded in the e12 cross-primitive closeout §11.6) |
