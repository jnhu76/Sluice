...
| audit #162 MODEL-007 remaining unmodeled mechanisms (items **(a)**, **(b)**, **(d)** are now modeled: (a) the I47-F2 `suspend_switch_pending` steal-refusal window — `spec/tla/e8_suspend_switch/`, issue #172, merged via PR #173; (b) the Phase-F1 WaitRecord registry generation/lease/delivery races — `spec/tla/f1_wait_record/`, issue #174, merged via PR #175 (safety PASS + 3 exact negatives + 6 witnesses); (d) the #115 spawn-to-busy-worker wake-epoch obligation — `spec/tla/spawn_wake_epoch/`, issue #176, safety PASS + 2 exact historical negatives + 6 witnesses): CancelToken task-cancel epoch protocol (`cancel.cpp`); G1 worker retire-ring ticket rescue (local_runnable→pending_spawn_) — umbrella tracker: issue #171 | any change to the corresponding mechanism |
