...
| Scheduler/backend wake convergence (park domains, epoch, split-wait bridge, R1–R4) | `src/async/scheduler_park_wake.cpp`, `scheduler.cpp` (park commit, `signal_wake_locked`, bridge) | e9-park-wake | Inv2/4/6/8/9/10 + Life1/3/5 + `InvNoCauselessReturn` (#185, in `Inv`, both configs) | Life2/4/7/8 under worker/self fairness (producers deliberately unfair — justified) | DrainParks (temporal), MixedSource, PrePark (pre-Phase-G snapshots), NoBridge (one-line Phase-G mutant), WitnessRetire/WitnessTerminate/WitnessPnpExit/WitnessPnpEndedRun (non-vacuity #189/#191), NegRetireDead/NegParticipantDead (fail-closed), WitnessRefObservation/WitnessRefUnbounded/WitnessRefDrainMWS3 (non-vacuity #185), NegOldEscape/NegNaiveEscape (fail-closed #185) | scheduler park/wake deterministic tests, `threadpool_wait_drain_deadlock_test` | REPAIRED, then **vacuity characterized 2026-08-23 (issue #185, B6 STOP) and REPAIRED at #185**: the unconditional `~SplitWait` LeavePark disjunct was mechanically demonstrated unfaithful to the as-built park (`scheduler_park_wake.cpp:400-468` — only an entry-armed observation or deadline bounds the park); the #185 repair scopes the reference escape to entry-armed parks (`observationArmed[w]` ghost), scopes `InvLife1DrainNoMW3Park` to armed parks, adds the `InvNoCauselessReturn` detector, three reachability witnesses, and two EXACT fail-closed negatives (19/19 gates); **#189 (2026-08-23, merged 921dd86)**: `RetireWorkerQuiescent` revived to the as-built retire epilogue; **#191 (merged 8c08e46)**: `ParticipantNoProgressExit` revived; see the debt register below and the suite README |
...
- **V3 — weak-memory kernels** (extract the real production atomic order
  ...
  field and carries no cross-thread ordering claim), and the RequestSlot
