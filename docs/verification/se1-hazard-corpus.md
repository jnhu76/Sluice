...
- Source roles: provenance sources are structured `{url, role, authority}` records with closed
  ...
  `bug_record` AND an `upstream_fix` source, etc.). Whether a URL's content truthfully matches its
  role, and whether the semantic mapping is valid, remains **HUMAN SEMANTIC PROVENANCE REVIEW**.
...
| Sluice-induced population cases | 8 | 6 production-runtime, 2 test-only |
| Probe companions (artifacts, excluded) | 1 | SE1-CA-H01-2 → `same_case_as` SE1-CA-H01-1 |
| OUT-OF-SE1 (preserved, visible) | 5 | 2 production-runtime growth defects, 2 structural-authority, 1 experiment-process |
...
| `select-event-registry-termination-vs-plain-wait-residency-unconfirmed` | liveness-reconciliation | SB-10 (UNKNOWN — key names the suspect instance, not a confirmed cause) |

OUT-OF-SE1 induced records (SB-03/SB-04, SB-11/SB-12, SB-13) stay outside the denominator; their
classes are described in §3.
...
| PARTIAL | 9 | H01, H03, H07, H11, H13 (+ induced SB-02, SB-05, SB-08, SB-10) |
...
- Schema frozen (`se1-corpus-schema` v1 as corrected in Draft; fail-closed validator PASS, wired
  into `scripts/gates/pre-push.sh` and CI).
