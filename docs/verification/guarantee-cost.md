...
| **bounded arena admission** (`would_block` refusal path) | pre-acceptance refusal with zero residue (C2d mutants M12/M13; D2-M7/M11) | **OVERLOAD-RELEVANT (measured)** | measured under sustained overload (§2): refusal p50 40 ns, p99 ≤ 60 ns — **cheaper than the accepted path** (refusal path: 1 arena-mutex lock + free-list empty check + counter++, no CAS, no slot write — `request_arena.hpp:242-254`); refusal share of sustained attempts ≈ 80 % at the operating point |
| **BlockingIoPool bounded dispatch storage** | no unbounded growth (AC-7 class implementation-resource tests) | STATIC (bounded) + RESCOPED | bounded by construction: fixed `worker_count` + `max_queue_depth` (`include/sluice/blocking_io_pool.hpp:48-57`), queue depth is the bounded dispatch storage; overload timing **not separately measured in this pilot** — rescoped to the static bound + the async arena admission proxy (see §0); recorded as an honest gap, not filled |
...
- BlockingIoPool dispatch-storage overload timing: rescoped out of pilot
  ...
  (`include/sluice/blocking_io_pool.hpp:48-57`) plus the async arena
