...
**Status: documented in CPPIO-CORE-006A.** This freezes the intended MVP
...
Reader  ── read_some (primitive)
         ...
         ├── read_vec          (derived vector primitive, CPPIO-CORE-005)
...
Writer  ── write_some (primitive)
         ...
         └── write_vec / write_all_vec  (derived vector, CPPIO-CORE-005)
...
(`Reader.zig:168` — `stream` writes `r.buffer[r.seek..r.end]` first). cppio
...
The buffered fast path (CPPIO-CORE-006C–006F) closes this gap **without** a
