...
**Status: CPPIO-CORE-015D.** Audit of the public header surface as of the
...
- `cppio::experimental::UringWriteBatch` + `UringWriteResult` (013).
- `cppio::experimental::UringIoContext` (013) — standalone, NOT a subclass of
  `cppio::IoContext`.
- `UringStats` lives in `cppio` (not experimental) only so bench/CSV helpers can
  treat it uniformly; it is still associated with the experimental uring path.

## Internal details (do not depend on these)

- **`cppio::detail::retry_on_eintr`** and everything under `include/cppio/detail/`
