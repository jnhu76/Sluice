...
**Status: CPPIO-CORE-014C.** How to run the experimental io_uring path on a
...
xmake build cppio_experimental_uring   # the experimental lib
```
...
- Link errors referencing `io_uring_*`: the experimental lib wasn't rebuilt with
  the flag — `xmake build cppio_experimental_uring` then rebuild dependents.
