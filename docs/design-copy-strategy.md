...
**Status: implemented in SLUICE-CORE-007A–007G.** The strategy boundary is now
...
> SLUICE-CORE-006 added an implicit buffered fast path inside `copy_all`.
> SLUICE-CORE-007 makes copy path selection explicit and testable.
...
- **Observability.** `CopyDecision` records which strategy was requested, which
  ...
  counts strategy selections. This is the observability SLUICE-CORE-010 will
  consume.
...
based on what the concrete reader/writer expose through their vtable. sluice's
...
throughput/latency numbers. **No benchmark exists yet** (SLUICE-CORE-010). The
