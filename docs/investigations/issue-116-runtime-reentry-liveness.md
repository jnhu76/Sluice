...
| `test:default-gate-targets` | 183 | default-gate test targets (== the `running.test` line count of a Linux Clang Debug stub `xmake test -v`); derived mechanically by `scripts/gates/mechanical-facts.py` from the xmake lua registrations, and every `test:default-gate-targets` doc row must equal it (live cross-reference: 167 → 168 on 2026-08-17, Issue #115 added `issue115_runnable_publication_wake_test`; 168 → 180 on 2026-08-18, the file-tools application track added 12 app tests; 180 → 181 on 2026-08-18, the performance-attribution round added `sluice_grep_matcher_differential_test`); 181 → 183 on 2026-08-19, the #135 Case B round added `failure_model_high_risk_test` + `failure_model_high_risk_death_test`) |
