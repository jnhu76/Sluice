...
for the B1-2+ integration slices. Detailed evidence, the happens-before
argument, the mutation matrix and the section 13 correspondence tables are in
...
| Implementation | `include/sluice/async/detail/request_core.hpp` + `src/async/detail/request_core.cpp`: a bounded, address-stable, mutex-serialized `RequestCore` (detail namespace) with reservation/acceptance/rollback, admission close, cancel arbitration, terminal selection with the admissibility floor, E/C pin acquisition and retirement, publication begin/complete with an outside-lock caller-target window, synchronous final-release reclaim, and generation advance to permanent retire. Canonical terminal representation is `sluice::detail::IoOutcome` (success with confirmed bytes, failure with confirmed prefix and effect certainty, `canceled_before_dispatch`). Tests: `tests/request_core_test_driver.hpp` (PublicationTarget with the ready release-store contract and post-ready publisher-touch detector; FakePhysicalDriver deterministic physical-event injection), `request_core_protocol_test.cpp` (29), `request_core_publication_test.cpp` (10), `request_core_consumer_probe.cpp` (2, incomplete-type host-neutrality asserts plus a public-surface lifecycle). Build: three additive targets in `xmake/tests.lua`, each compiling the substrate TU and linking only `sluice_core`. No AsyncIoContext/ThreadPool/io_uring/Scheduler/Fiber/WaiterToken/RoutingLease/ProgressSource wiring exists |
...
| Semantic/regression evidence | `request_core_protocol_test` 29/29 and `request_core_publication_test` 10/10 (debug and ASan+UBSan via the clang toolchain; the project-wide `asanubsan` mode cannot use gcc on this host because libasan is absent), `request_core_consumer_probe` 2/2, full suite 37/37 in this configuration, `scripts/check_cpp_comment_authority.py` clean. Load-bearing case names are enumerated in the evidence note (admission/close race, generation aliasing and exhaustion, terminal floor incl. pre-claim cancel and health flag, E-chain continuity, pin-gated reclaim with B/C/P-last variants, publication ordering, release racing the publication epilogue, effect-certainty representation for post-accept and pre-claim failures, zero-op) |
| Protocol/model evidence | `formal/tla/RequestCore.tla` (first liveness-carrying model; `FairSpec` with six `WF_vars` conjuncts on named service actions) + 16 cfgs gated by `scripts/verify_tla.sh` stage B1-1: clean safety (2,244,362 states generated / 399,890 distinct / 0 on queue, 13 invariants), clean liveness (`PROPERTY L1 L5` checked, no violation), 12 invariant-killed mutants and 2 temporal-killed mutants, each kill matched against the exact violated invariant line. Bounds: 2 slots, GenMax 2, MaxCtl 1. Full correspondence tables in the evidence note's section 11 |
| Publication/thread evidence | CPP_MEMORY_MODEL review recorded (evidence note section 5): core state exclusively mutex-guarded; the one lock-free pair is the caller target's ready release-store → consumer acquire, with the payload-visibility obligation stated; the release-racing-epilogue analysis covers both interleavings with the P pin and full-identity epilogue resolution; reclaim has a zero-gap `reclaimable ⇒ reclaimed` postcondition per release call. Cross-thread instrumentation and TSan are explicitly not claimed (deterministic single-thread tests only); multi-threaded publication evidence belongs to the public-surface/threading slice |
