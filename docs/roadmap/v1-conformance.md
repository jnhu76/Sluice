...
| Implementation | `AsyncIoContext` (the root's `IoContext` before the public rename) gains `std::unique_ptr<detail::RequestCore> core_`, allocated in its constructor and transferred untouched by both move operations, so the core object's address, slot table, generation state and identity survive a legal context move. New `sluice::async::detail::allocate_context_identity()` (`include/sluice/async/detail/context_identity.hpp`, `src/async/detail/context_identity.cpp`, listed in `xmake/libraries.lua` because the `src/async/*.cpp` glob misses `detail/`) is the single identity domain; `src/async/detail/request_core.cpp` joins the library for the same reason, since the context owns the core in production. New backend seam `AsyncBackend::adopt_context_identity(detail::ContextIdentity) -> std::size_t` (private virtual, `friend AsyncIoContext`, mirroring `resolve_identity_state`): the context hands over its identity and the backend installs it into its slot table and reports that table's capacity, which sizes the context core. `RequestArena` no longer receives an identity at construction and gains `adopt_context_identity`; `ThreadPoolBackend` and `UringAsyncBackend` implement the seam and expose `arena_context_identity()` beside the existing `arena_capacity()` diagnostic. Evidence target: `tests/request_core_ownership_test.cpp` |
| Change | Context construction now allocates one identity from the single domain, installs it into the backend slot table, and constructs the context-owned core with that identity and the reported capacity; a context move transfers the same core instead of re-homing state. The two backend-local identity namespaces (`next_backend_id()` with the magic bases `0x54500000` / `0x55720000`) are deleted; disjointness between backends is now structural (one counter) instead of conventional. `RequestHandle`, `resolve_identity_state`, `identity_of`, `supports_request_identity()`, `Completion` authority, the arena slot lifecycle and all publication behavior are unchanged. Production ThreadPool and io_uring requests still route exclusively through `RequestArena`: the core is idle for them (evidence below) |
| Semantic/regression evidence | `request_core_ownership_test` 6/6 (ThreadPool) and `request_core_ownership_uring_test` 7/7 (real ring), full suite 42/42 with `--liburing=y` in debug and release, 38/38 in debug without liburing, targeted ASan+UBSan with leak detection over both ownership binaries (6/6 and 7/7; the project-wide `asanubsan` mode still cannot link gcc on this host because libasan is absent), `scripts/check_cpp_comment_authority.py` clean. Load-bearing cases: `move_transfers_the_same_core` (same core object, identity and accepted slot state across a context move; the moved-from context holds no second core; core slot budget equals the backend slot table capacity), `backend_slot_table_carries_the_context_identity` (two backends constructed without a context share one unadopted slot-table identity; each adopted slot table carries its own context's identity; distinct contexts hold distinct identities), `foreign_and_released_identities_do_not_resolve` (own context resolves, foreign context rejects, empty handle rejects, identity survives a context move, a released identity stops resolving), `production_request_is_not_request_core_owned` (backend slot in use and terminal ready while every core fact stays zero: no mirrored reservation, acceptance, terminal, publication or binding; the core admission gate is untouched), `adoption_seam_carries_no_lifecycle_authority` (a recording backend adopts nothing at construction, adopts exactly once at context construction, the core carries the identity the backend received plus the capacity it reported, and neither adoption nor the move changes a canonical core fact), `domain_identities_are_not_reused_across_contexts` (eight live contexts, all nonzero, pairwise distinct), `uring_context_carries_the_context_identity` (same identity, capacity and no-mirror facts through the real io_uring backend) |
| Mutation evidence | Five production mutations were applied, force-rebuilt and run against `request_core_ownership_test`; each was killed by a named assertion and the unmutated baseline passed after restore. The force-rebuild is per-target and that target links no library object, so the stub-archive hazard above does not apply to these runs. Move re-homes a fresh core → `move_transfers_the_same_core`. Slot table ignores the adopted identity → `backend_slot_table_carries_the_context_identity`. Identity domain returns a constant → `backend_slot_table_carries_the_context_identity` and `domain_identities_are_not_reused_across_contexts`. Core budget hardcoded instead of the reported capacity → `move_transfers_the_same_core`, `backend_slot_table_carries_the_context_identity`, `adoption_seam_carries_no_lifecycle_authority`. Production acceptance mirrored into the core → `production_request_is_not_request_core_owned` |
...
| Publication/thread evidence | No new synchronization and no changed happens-before edge. The identity is written into the backend slot table inside the context constructor, before the context is reachable by any other thread, so no request can observe an unadopted or half-adopted slot table; the context move remains the pre-existing `unique_ptr` transfer under the caller's normal external synchronization. Adoption returns no owner, borrows nothing, and the backend never receives the core, so no delivered interleaving is affected. Cross-thread instrumentation and TSan are not claimed; deterministic single-thread evidence only, as in B1-1 |
| Backend/kernel evidence | ThreadPool evidence is a real backend with a live worker. io_uring evidence uses the real kernel ring (`UringAsyncBackend::available()` true on this host, liburing 2.13); when no ring is available the io_uring case reports `NOT RUN` instead of passing. The io_uring case reads a real file; no filesystem-behavior claim is added and the A1-recorded io_uring gaps stay with #400. Reproduction note: the local `xmake -r` rare rebuild can leave `libsluice_async.a` built from the no-liburing stub objects; that reproduces on the base commit, is unrelated to this slice, and is avoided by an ordinary build or `xmake clean -a` |
