...
| Common logical API | blocking common File surface 已收敛（A1 positional + A4 sequential + sync_data）；evented common surface 以显式 `await_* + Completion` 形式存在，是否需要进一步统一收敛未裁决 | `PARTIAL` | evented common surface 的最小 spelling 须凭 consumer 证据单独裁决 |
