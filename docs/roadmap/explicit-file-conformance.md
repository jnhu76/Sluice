...
| Explicit outstanding API resource reference | explicit op structs（`ReadOp`/`WriteOp`/`SyncDataOp`/`SyncAllOp`）经 `NativeFileRef` 引用资源：从 canonical `File` 隐式转换为常规拼写（`ReadOp{file, ...}`），机制级 native handle 保持为显式命名的 `NativeFileRef{int}` interop 路径（bare int 不再构造 op）；`NativeFileRef` 不拥有所有权/lifetime/close authority；两个 backend 的 lowering 不变，runtime 机制保持 File-agnostic | `CONFORMING` | 保持零所有权引用；不得引入 registry / shared pin / runtime File knowledge；interop 资源继续经显式命名 `NativeFileRef{int}` 表达 |
