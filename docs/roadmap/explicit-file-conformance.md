...
- **Verified implementation baseline**: `cfbbb3158283ed202d4b5064999ac7c52f69cfb2`（该 commit 可 checkout 并验证下文描述的实现状态）
...
Verified implementation baseline：`cfbbb315`（A1 + A2 + A3 implementation；下表状态在该 commit 上成立）。
...
在 `cfbbb315`（verified implementation baseline；A3 implementation conforming，非 issue 关闭）：
