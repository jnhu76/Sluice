...
| -- | ------------ | ------- | --------------- | --------------------- | ----------------- | -------------- | --------- | -------- | ------ |
| PF-001 | sluice-grep, sparse patterns | V1 per-line `std::search` matcher dominated wall time | yes (V2 chunk scan; dense-anchor emit cost remains — see Detail and PF-004) | n/a (APP layer) | bytes-class (`T≈α·bytes`, α improved 2.4–3.1×) | APP | APP-local (done) | `docs/results/performance-attribution/round1-grep-*-ladder.json`; differential oracle test | APP_FIXED |
| PF-002 | sluice-grep, all workloads | aggregate Core increment L4−L3 ≈ 48–67 ms/GiB on sparse/binary rows @ 1 MiB chunks (107–150 on dense-anchor/short rows; per-workload values in the artifact `derived`) | yes | Core share/overhead ratio per workload in ladder `derived` | NOT yet characterized (needs buffer-size/request-count sweep) | INVESTIGATE (internal composition unproven — hypothesis: per-request fixed cost) | TBD (pending decomposition) | `docs/results/performance-attribution/round1-grep-v2-ladder.json` | INVESTIGATE |
...
### PF-001 — grep sparse matcher (APP_FIXED)
...
cursor, borrow-free SWAR newline count) improved sparse rows 2.4–3.1× and
binary rows 2.8× with byte-identical semantics (differential oracle
against the frozen V1 reference). Dense/common-anchor rows remain
emit-bound — that follow-up is tracked as PF-004 (open).
...
diagnosis, NOT implemented): deduped clock queries for waits without
deadlines — it touches Scheduler wait semantics and belongs behind the
