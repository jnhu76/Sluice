...
- **Reopen condition**：真实 consumer 需要 multi-buffer canonical I/O，且其 correctness 依赖于相关 OS 对象上 readv/writev 单次 syscall 所提供的条件性 single-operation / non-interleaving 属性（如 O_APPEND 使整组追加原子地落在文件尾、pipe 上总量 ≤ PIPE_BUF 的原子写；属性是否存在取决于对象类型、操作与大小等条件），而 scalar composition 无法表达/保持该性质（单纯的 syscall 计数或性能考量不构成"不足"，性能归 Phase B 实证）→ 开新的 narrow node。这些 OS mechanism 属性并未被 Sluice 冻结为 canonical public semantic contract；任何重新引入仍须满足 ADR-0002 §5.3 的 evidence 与 parity 要求。届时 async 与 sync 必须共享同一 vectored semantics（ADR-0002 §5.3），不允许长期 sync-vector / async-scalar 两套语义世界。
