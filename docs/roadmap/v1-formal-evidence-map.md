...
| MC-2 RequestArena/Completion slot FSM unmodeled (terminal arbitration, deferred publication, exactly-once) | Still valid, remapped (surface becomes context-owned RequestCore) | REQ-01–REQ-05, HANDLE-01/02, VERIFY-03 L1/L5 | #394 (+#395 public binding) | Lifecycle TLA+ model + deterministic C++ interleavings + mutation configs (section 9). The #394 lifecycle half closed in the B1-1/B1-B/B1-C records; #395's public-binding half — consumption atomic with binding release, the non-consuming observation, and the always-on destruction diagnostics — is implemented with evidence in the B2 review record (model `Consume` + killed consumption mutants + C++ public-request suites) |
