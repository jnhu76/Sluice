...
Result<T> (caller thread blocks)            caller observes completion
    ↓                                          ↓
blocking::sync_data / blocking::sync_all  |  await_sync_data / await_sync_all
    ↓
File::close
```

Read / Write / SyncData / SyncAll 的落地没有要求 Scheduler、Completion、RequestArena 获得 File semantic authority。
