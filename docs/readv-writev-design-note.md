...
**Status: prototype implemented in CPPIO-CORE-005.** The default fallback, the
...
| Piece | Location |
|---|---|
| `IoSlice` / `ConstIoSlice` slice types | `include/cppio/iovec.hpp` |
| `Reader::read_vec` default fallback | `include/cppio/reader.hpp`, `src/reader.cpp` |
| `Writer::write_vec` default fallback | `include/cppio/writer.hpp`, `src/writer.cpp` |
| `Writer::write_all_vec` derived helper | `src/writer.cpp` |
| `FileReader::read_vec` / `FileWriter::write_vec` POSIX overrides | `src/file.cpp` |
| `VectorStats` struct | `include/cppio/measurement.hpp` |
...
- No io_uring (scheduled for CPPIO-CORE-012).
- No async / thread-pool backend.
- No benchmark integration (CPPIO-CORE-010). No performance conclusion.
- No zero-copy buffered fast path (CPPIO-CORE-006): the default fallback still
  routes each slice through `read_some`/`write_some`.
- No copy strategy layer (CPPIO-CORE-007).
...
exact-read/exact-write helpers. Final semantics (CPPIO-CORE-005B):
...
cppio currently propagates vector errors **immediately**, even after partial
...
- The `open_error_` behavior from CPPIO-CORE-002 is preserved: a failed-open
...
`VectorStats` (`include/cppio/measurement.hpp`):
...
Stats are caller-owned, never global (same rule as CPPIO-CORE-004). A null
...
- No microbenchmark exists yet (CPPIO-CORE-010). The relative cost of the gather
...
measuring.** CPPIO-CORE-010/011 will produce the decision matrix.

## 8. Relation to Zig `std.Io`

Zig `std.Io.Reader`/`Writer` provide `readVec`/`readVecAll`/`writeVec`/
`writeVecAll` over `[][]u8` / `[]const []const u8`. cppio's `read_vec`/
...
- Zig's vector primitives negotiate around the **interface-owned buffer** inside
  each Reader/Writer. cppio has no internal buffer (external `BufferedReader`/
  `BufferedWriter` wrappers — see `docs/zig-std-io-gap-calibration.md` §4.1), so
  the fallback loops directly over `read_some`/`write_some`.
- Zig's `readVec` swallows `EndOfStream` when bytes were already read
  (`if (n == 0) return error.EndOfStream else 0`). cppio instead models EOF as a
...
- cppio has no async / uring / cancellation; Zig's `std.Io` is built around an
