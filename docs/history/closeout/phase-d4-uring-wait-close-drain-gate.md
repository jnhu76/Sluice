...
| wait-source `mtx_` (LEAF) | progress/control epochs, eventfd drain, committed-wait registration | snapshot; epoch check + drain; arm_committed_wait | call Scheduler; publish Completions; call user code; touch request state; ACQUIRE the Scheduler `global_mtx_` (the one edge is the reverse: `global_mtx_ -> wait-source mtx_` for `arm_committed_wait()`, D4-RM14); block on asynchronous progress (a BLOCKING syscall would violate the leaf); call Scheduler/user/sink/request-lifecycle code. EXCEPTION: `drain_eventfd_nolock_()` performs a NONBLOCKING `::read(control_fd_)` drain loop while holding `mtx_` — this is the one allowed in-lock syscall. eventfd(2) created with `EFD_NONBLOCK` returns `EAGAIN` instead of blocking on an empty counter, the loop drains only the bounded counter and never blocks, and `eventfd` is MT-Safe, so it cannot deadlock or wait on asynchronous progress. No other syscall may run under `mtx_`. |
