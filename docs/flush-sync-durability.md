...
**Status: contract documented in CPPIO-CORE-008A; implemented in 008B–008G.**
...
CPPIO-CORE-008 adds explicit sync/durability semantics on top of the existing
...
owned buffer into the underlying sink — the same concept as cppio's
...
separately. cppio mirrors that separation: `flush()` drains, `SyncableWriter`
syncs. See `docs/zig-std-io-gap-calibration.md`.
...
matters for a given workload is unmeasured until CPPIO-CORE-010. No claim is
