...
> Historical conformance to ADR-0002. CLOSED/CONFORMANT labels below do not certify the new v1 target. Use [the v1 ledger](../../roadmap/v1-conformance.md) for current convergence.
...
- **Original authority (historical only)**: [`docs/mission.md`](../architecture/mission.md) → [`ADR-0001`](../../adr/0001-explicit-io-design-doctrine.md) → [`ADR-0002`](../../adr/0002-explicit-file-api-architecture.md)
...
Verified implementation baseline：`d7511349`（A1 + A2 + A3 + A4 + A5 + A6 implementation；下表状态在该 commit 上成立）。
...
| Common logical API | blocking common File-facing surface 覆盖 Phase-A 保留操作（A1 positional + A4 sequential + sync_data/sync_all + A3 state）；evented File-facing common forms 仅存在于 positional I/O 与 SyncData/SyncAll；sequential 与 state（size/resize）的 evented 形式因无 consumer 证据而**有意不存在** | `PARTIAL` | `PARTIAL` 不等于 evented parity：上表之外（含 evented sequential、evented state）一律未赚取，须凭 consumer 证据单独裁决，不得默认存在 |
| Explicit outstanding API resource reference | explicit op structs（`ReadOp`/`WriteOp`/`SyncDataOp`/`SyncAllOp`）经 `NativeFileRef` 引用资源：从 canonical `File` 隐式转换为常规拼写（`ReadOp{file, ...}`），机制级 native handle 保持为显式命名的 `NativeFileRef{int, declared access}` interop 路径（bare int 不再构造 op）；`NativeFileRef` 不拥有所有权/lifetime/close authority；两个 backend 的 lowering 不变，runtime 机制保持 File-agnostic | `CONFORMING` | 保持零所有权引用；不得引入 registry / shared pin / runtime File knowledge；interop 资源继续经显式命名 `NativeFileRef{int, declared access}` 表达 |
| §5.5 admission precedence | frozen precedence（closed > access legality > zero-length > offset/size 校验）的 guard 顺序存在于全部 initiation surface（blocking sequential/positional/state、evented File-facing、explicit op submission 及 request variants）；collision-level executable witnesses（`tests/file_access_precedence_test.cpp`；uring lowering 的 zero-length-before-offset 由 liburing smoke witness 钉住）钉死多项条件同立时的 caller-visible 结果，并以 backend-entry count=0 witness 绑定"拒绝发生在 backend 之前"；本 corrective 对抗审计中实际执行的 targeted guard-reorder/guard-deletion mutations 均被对应 collision witness 杀死 | `CONFORMING` | 保持 guard 顺序；任何改变 observable precedence 的 reorder 必须失败于该测试 |
...
| Vectored Read / Write | decision 已记录于 [`explicit-file-vectored-decision.md`](explicit-file-vectored-decision.md)：vectored 未被 earn 为 canonical Phase A surface（app census zero 使用；唯一 in-tree 消费链根植于 external-consumer 为零的 WAL）；legacy composition-layer surface 保留，待 legacy-surface audit 裁决；promotion 需要新的 consumer 证据 | `CONFORMING` | 架构满足恰恰以 NOT carrying 未 earn 的 surface 达成；gated extension 保持 gated（ADR-0002 §5.3）；reopen 需真实 consumer 证明 scalar composition 不足 |
...
| App canonical-resource consumption | hash/grep/tail 已迁到 canonical `File`（`File::open` 所有权 + `await_read_at`）；copy source lifetime 亦为 canonical `File`（outcome 持有，`native_handle()` 只在 fstat 观察读取；pipeline 边界直接传 `File` 引用）；copy pipeline 保持 explicit outstanding authority；全部残余 escape 已分类（`docs/roadmap/explicit-file-app-consumer-census.md`：interop / namespace work） | `CONFORMING` | 每个已被 canonical `File` 表达的资源 lifetime 都由 `File` 持有；剩余 raw/native escape 仅因必要的 metadata、resize/durability 或 namespace authority 位于别处。`CONFORMING` 不要求 zero `native_handle()`，只要求 zero unclassified bypass |
...
在 `d7511349`（verified implementation baseline；A2–A6 implementation conforming，非 issue 关闭）：
...
Explicit low-level File ref   CONFORMING (A6)
Vectored decision             DECIDED — not carried as canonical Phase A surface (A7)
```
