# Zig `std.Io` ↔ sluice gap calibration
...
This document tracks how faithfully the sluice C++ core mirrors Zig `std.Io`'s
...
to close them. No performance claims — measurements land in SLUICE-CORE-010/011.

## 2. Mapping table

| Zig `std.Io` idea | sluice equivalent | Fidelity |
|---|---|---|
| `Reader` (vtable + interface-owned buffer) | `sluice::Reader` (virtual, **no** internal buffer) | partial — see §4.1 |
| `Writer` (vtable + interface-owned buffer) | `sluice::Writer` (virtual, **no** internal buffer) | partial — see §4.1 |
...
| `Reader.peek` / buffered fast path | `BufferedReadable` + `copy_all` fast path | **Partial / Improved in SLUICE-CORE-006** — see §4.2 |
| `readv`/`writev` (vector I/O) | `read_vec`/`write_vec`/`write_all_vec` + POSIX overrides | **Partial / Improved in SLUICE-CORE-005** — see §4.6 |
...
around the buffer. sluice deliberately uses **external** `BufferedReader`/
`BufferedWriter` wrappers (approved by the SLUICE-001 design table). This makes
Zig's `rebase`, `preserve_len`, and the `Discarding`/`Allocating` writers have
no direct analog here. Approved divergence.

### 4.2 Copy fast path: partial — improved in SLUICE-CORE-006
...
— a zero-copy fast path because the buffer lives inside the reader. sluice's base
...
sluice splits it: `FileWriter::flush()` is a documented no-op (no `fsync` this
phase), `BufferedWriter::flush()` does the work. Callers must know which layer
is buffered. See `README.md` "Flush contract" and SLUICE-CORE-008.

### 4.4 Error model flattened
Zig: `Reader.Error = {ReadFailed, EndOfStream}`, `Writer.Error = {WriteFailed}`;
backend detail lives in the implementation. sluice's `IoError::Code` carries
...
### 4.6 Vector I/O: partial fidelity, improved in SLUICE-CORE-005
SLUICE-CORE-005 added `read_vec`/`write_vec`/`write_all_vec` plus POSIX
...
- **Buffer model.** Zig's vector primitives negotiate around the
  interface-owned buffer inside each Reader/Writer; sluice's default fallback
  loops directly over `read_some`/`write_some` (no internal buffer — §4.1).

sluice's error model on partial progress is also stricter by decision: errors are
...
sluice now has vector primitives. It still does not have Zig's full
...
### 4.7 Copy strategy: explicit layer vs Zig's vtable negotiation — improved in SLUICE-CORE-007
SLUICE-CORE-007 improves the strategy boundary but does not make sluice equal to
...
behavior based on what the concrete reader/writer expose). sluice's base
...
the caller selects. This is an intentional C++ design divergence: sluice prefers
...
and from which `Reader`/`Writer` are created. sluice cannot adopt that model
...
SLUICE-CORE-004 added the measurement structs (`SyscallStats`, `BufferStats`,
`CopyStats`) and SLUICE-CORE-005 added `VectorStats`; all are wired through
...
unmeasured until SLUICE-CORE-010: actual wall-clock throughput/latency, and the
...
|---|---|---|
| ~~Buffered fast path (zero-copy copy)~~ | ~~SLUICE-CORE-006~~ | **done in 006 (partial — see §4.2)** |
| ~~`readv`/`writev` vector I/O~~ | ~~SLUICE-CORE-005~~ | **done in 005 (partial — see §4.6)** |
| ~~Copy strategy layer~~ | ~~SLUICE-CORE-007~~ | **done in 007 (explicit boundary — see §4.7)** |
| ~~Flush/sync/durability split~~ | ~~SLUICE-CORE-008~~ | **done in 008 — see docs/flush-sync-durability.md** |
| ~~`IoContext` capability object~~ | ~~SLUICE-CORE-009~~ | **done in 009 (BlockingIoContext) — see §4.8** |
| io_uring | SLUICE-CORE-012 | requires 004–011 preconditions (now met) |
