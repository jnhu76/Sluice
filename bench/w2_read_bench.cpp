...
void run_stream_offsets(sluice::FileReader& shared, std::uint64_t base_offset,
                        std::size_t blocks_per_stream, std::size_t block_size, std::byte* scratch) {
    std::span<std::byte> buf(scratch, block_size);
...
std::uint64_t run_sequential(const Params& pm, std::byte* scratch,
                             const std::vector<std::string>& paths, sluice::FileReader* shared) {
    auto t0 = now_ns();
...
std::uint64_t run_bounded_pool(const Params& pm, const std::vector<std::string>& paths,
    ...
    for (std::size_t s = 0; s < pm.streams; ++s) {
        ...
        if (pm.file_layout == "many_files") {
            ...
            pool.submit([shared, base, &pm, sc] {
                run_stream_offsets(*shared, base, pm.blocks_per_stream, pm.block_size, sc);
            });
        }
    }
    pool.wait_all();
...
std::uint64_t run_thread_per_stream(const Params& pm, const std::vector<std::string>& paths,
                                    sluice::FileReader* shared) {
    std::vector<std::vector<std::byte>> scratches(pm.streams,
                                                  std::vector<std::byte>(pm.block_size));
...
void run_cell(const Params& pm) {
    ...
    emit(pm, "blocking_sequential", run_sequential(pm, sc, paths, shared.get()), 1);
    emit(pm, "blocking_bounded_pool", run_bounded_pool(pm, paths, shared.get()), pm.pool_threads);
    emit(pm, "blocking_thread_per_stream", run_thread_per_stream(pm, paths, shared.get()),
         pm.streams);
}
