...
#define SLUICE_REQUIRES_SHARED(...) SLUICE_TSA_ATTR__(requires_shared_capability(__VA_ARGS__))
...
#define SLUICE_ACQUIRE_SHARED(...) SLUICE_TSA_ATTR__(acquire_shared_capability(__VA_ARGS__))

#define SLUICE_RELEASE(...) SLUICE_TSA_ATTR__(release_capability(__VA_ARGS__))

#define SLUICE_RELEASE_SHARED(...) SLUICE_TSA_ATTR__(release_shared_capability(__VA_ARGS__))

#define SLUICE_TRY_ACQUIRE(...) SLUICE_TSA_ATTR__(try_acquire_capability(__VA_ARGS__))

#define SLUICE_TRY_ACQUIRE_SHARED(...) SLUICE_TSA_ATTR__(try_acquire_shared_capability(__VA_ARGS__))

#define SLUICE_ASSERT_CAPABILITY(...) SLUICE_TSA_ATTR__(assert_capability(__VA_ARGS__))

#define SLUICE_ASSERT_SHARED_CAPABILITY(...)                                                       \
    SLUICE_TSA_ATTR__(assert_shared_capability(__VA_ARGS__))

#define SLUICE_NO_THREAD_SAFETY_ANALYSIS SLUICE_TSA_ATTR__(no_thread_safety_analysis)

#define SLUICE_RETURN_CAPABILITY(...) SLUICE_TSA_ATTR__(lock_returned(__VA_ARGS__))
