...
本阶段聚焦正确性、可观测性和可组合性。优化、基准测试、io_uring、异步后端和策略选择被有意推迟。
...
异步 I/O **设计**（sluice-CORE-016）已作为纯设计文档落地：一份已接受的 ADR（`docs/adr/ADR-async-io-model.md`），规定了基于 completion、仅文件 I/O、可选启用的异步基础。**目前尚无任何异步代码**——阻塞仍是默认且唯一已实现的后端。

异步**实现**被刻意阻挡在**同步优先就绪门控**（`docs/sync-before-async-readiness-gate.md`）之后：阻塞基线必须先补齐位置型 I/O、耐久性模型、有界线程池基线以及 W1–W4 基准矩阵（任务 017S–023S，`docs/sync-io-next-jobs.md`），以便将来把异步与一个工程化的、并发的阻塞基线对比，而非仅与顺序阻塞对比。理由见 `docs/sync-io-model-gap-audit.md`。
